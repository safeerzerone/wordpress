...
if ( ! class_exists( 'ARM_restriction_Lite' ) ) {
	class ARM_restriction_Lite {
			...
			if ( $hide_wp_login == 1 ) {
				...
				if ( in_array( $GLOBALS['pagenow'], $hide_pages ) || in_array( $pagenow, $hide_pages ) ) {
					...
					if ( isset( $_GET['arm-key'] ) || isset( $_GET['key'] ) || ( isset( $_GET['action'] ) && in_array( $_GET['action'], array( 'logout', 'armrp', 'resetpass', 'lostpassword', 'retrievepassword', 'postpass' ) ) ) ) { //phpcs:ignore
						...
						$interim_login = ( isset( $_REQUEST['interim-login'] ) && $_REQUEST['interim-login'] == 1 ) ? true : false; //phpcs:ignore
		...
		function arm_restrict_all_access( $wp ) {
			...
			if ( true === ( is_admin() || is_user_logged_in() || current_user_can( 'administrator' ) || ( defined( 'WP_INSTALLING' ) && isset( $_GET['key'] ) ) ) ) { //phpcs:ignore
				return;
			}
			...
			if ( ! is_admin() && ! current_user_can( 'administrator' ) ) {
				if ( ! empty( $current_user->deny_term ) ) {
					...
					if ( is_array( $excluded_array ) ) {
						$args['exclude'] = array_merge( $excluded_array, $hide_terms ); // phpcs:ignore
					} else {
						if ( empty( $excluded_array ) ) {
							...
							$args['exclude']             = implode( ',', $exploded_new_excluded_terms ); // phpcs:ignore
						}
					}
				}
			}
		...
		function arm_widget_posts_args( $args ) {
			...
			if ( ! is_admin() && ! current_user_can( 'administrator' ) ) {
				...
				$args['post__not_in'] = $restrict_posts; // phpcs:ignore
			}
		...
		function arm_widget_pages_args( $args ) {
			...
			if ( ! is_admin() && ! current_user_can( 'administrator' ) ) {
				...
				$rargs = array(
					'post_type'  => 'page',
					'meta_key'   => 'arm_access_plan', // phpcs:ignore
				...
				$args['exclude'] = implode( ',', $restrict_pages ); // phpcs:ignore
			}
	...
	class ARM_fail_attempts {
		...
		function logFailAttempts( $username = '', $user_id = 0 ) {
			...
			if ( $failed_login_lockdown == 1 ) {
				...
				$wpdb->insert( $ARMemberLite->tbl_arm_fail_attempts, $ins_data ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
			}
		}
		...
		function lockDown( $username = '', $lock_minutes = 0, $user_id = 0 ) {
			...
			$relase_lock = date( 'Y-m-d H:i:s', strtotime( $now . ' + ' . $lock_minutes . ' minute' ) ); //phpcs:ignore
			$ins_data    = array(
				'arm_user_id'       => $user_id,
			...
			$wpdb->insert( $ARMemberLite->tbl_arm_lockdown, $ins_data ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
