...
class arm_latest_user_list_element_shortcode extends Widget_Base
{
    ...
    {
        return esc_html('ARMember latest user list','armember-membership').'<style>
        .arm_element_icon{
    ...
    protected function register_controls()
    {
        ...
        $this->start_controls_section(
			'section_content',
			[
				'label' => esc_html__( 'ARMember Latest Users', 'armember-membership' ),
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Total Records To Display', 'armember-membership' ),
				'type' => Controls_Manager::TEXT,
				'label_block' => true,
				'default'=>'5',
			]
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Display Type', 'armember-membership'),
				'type' => Controls_Manager::SELECT,
				'default' => 'list',
				'options' =>[
					'list'=> esc_html__('List','armember-membership'),
					'slider'=> esc_html__('Slider','armember-membership')
				],
				'label_block' => true,
		...
		$this->add_control(
			...
			[
				...
				'options' =>[
					...
					'crossfade'=> esc_html__('Cross Fade','armember-membership'),
	...
	{
		global $wp, $wpdb, $current_user, $arm_errors, $ARMemberLite, $arm_global_settings, $arm_subscription_plans, $arm_member_forms, $arm_members_activity,$is_globalcss_added,$arm_social_feature,$arm_members_directory;
		...
		$arg = array(
			...
			'number' => ($total_rec + 10),
			'fields' => 'all',
							'meta_key'     => 'arm_primary_status', //phpcs:ignore
		...
		if(!empty($frontfontstyle['google_font_url'])){
			$output .= '<link id="google-font" rel="stylesheet" type="text/css" href="' . $frontfontstyle['google_font_url'] . '" />'; //phpcs:ignore
		}
				...
				$output .= "<div class='arm_slider_widget_wrapper arm_widget_wrapper_{$random_inner_key}'>";
					...
					$output .= "<div class='arm_slider_widget_header'>";
						...
						$output .= "<div class='arm_slider_widget_user_cover'>";
							$output .= "<img src='{$profile_cover}' style='width:100%;height:100%;border-radius:0;-webkit-border-radius:0;-o-border-radius:0;-moz-borde-radius:0;' />"; //phpcs:ignore
						$output .= "</div>";
