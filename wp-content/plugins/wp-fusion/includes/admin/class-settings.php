...
class WPF_Settings {
	...
	public function show_field_oauth_connection_status( $id, $field ) {
		...
		if ( ! empty( $access_token ) && ! empty( $refresh_token ) ) {
			...
			// Translators: %s is the name of the CRM.
			printf( esc_html__( 'Connected to %s', 'wp-fusion' ), esc_html( $field['name'] ) );
		} else {
			echo '<span class="dashicons dashicons-warning"></span>';
			esc_html_e( 'Not Connected', 'wp-fusion' );
		}
		...
		$tip = sprintf( __( 'Refresh all custom fields and available tags from %s. Does not modify any user data or permissions.', 'wp-fusion' ), $field['name'] );
		...
		if ( ! empty( $field['url'] ) ) {
			...
			printf( esc_html__( 'Re-authorize with %s', 'wp-fusion' ), esc_html( $field['name'] ) );
			echo '</a>';
		}
		...
		echo '<span class="toggle-text">' . esc_html__( 'Show tokens for debugging', 'wp-fusion' ) . '</span>';
		echo '</a>';
		echo '</div>';
		...
		echo '<p style="margin-top: 0; color: #666; font-style: italic;">' . esc_html__( 'Token fields for debugging purposes. These are automatically managed by the OAuth connection.', 'wp-fusion' ) . '</p>';

		// Access token field
		if ( $access_token_field ) {
			echo '<div style="margin-bottom: 15px;">';
			echo '<label for="' . esc_attr( $access_token_field ) . '" style="display: block; font-weight: bold; margin-bottom: 5px;">' . esc_html__( 'Access Token', 'wp-fusion' ) . '</label>';

			// Handle non-scalar values
			if ( ! is_scalar( $access_token ) ) {
				echo '<strong style="color: #dc3232;">' . esc_html__( 'Error: The access token is not a string. Please reauthorize the connection.', 'wp-fusion' ) . '</strong>';
				$access_token = '';
			}
		...
		if ( $refresh_token_field ) {
			echo '<div>';
			echo '<label for="' . esc_attr( $refresh_token_field ) . '" style="display: block; font-weight: bold; margin-bottom: 5px;">' . esc_html__( 'Refresh Token', 'wp-fusion' ) . '</label>';

			// Handle non-scalar values
			if ( ! is_scalar( $refresh_token ) ) {
				echo '<strong style="color: #dc3232;">' . esc_html__( 'Error: The refresh token is not a string. Please reauthorize the connection.', 'wp-fusion' ) . '</strong>';
				$refresh_token = '';
			}
	...
	public function show_field_export_options( $id, $field ) {
		...
		foreach ( $options as $key => $data ) {
			...
			if ( isset( $batch_timestamps[ $key ] ) ) {
				...
				echo '(' . esc_html__( 'Last run:', 'wp-fusion' ) . ' ';
	...
	public function handle_disconnect_crm() {
		...
		wp_send_json_success( __( 'CRM connection disconnected successfully.', 'wp-fusion' ) );
	}
}
