...
class WPF_FooEvents extends WPF_Integrations_Base {
	...
	public function init() {
		...
		add_action( 'added_post_meta', array( $this, 'sync_ticket_number' ), 10, 4 );
		add_action( 'updated_post_meta', array( $this, 'sync_ticket_number' ), 10, 4 );
	...
	/**
	 * Formats a FooEvents date according to WordPress date format settings.
	 *
	 * @since 3.46.10
	 *
	 * @param string $date_string The raw date string from FooEvents.
	 * @return string Formatted date string in Y-m-d format for CRM integration.
	 */
	private function format_event_date( $date_string ) {
		...
		// Try to create a DateTime object using WordPress's date format.
		$date_obj = DateTime::createFromFormat( $wp_date_format, $date_string );

		// If we still don't have a valid date, log and return original.
		if ( false === $date_obj ) {
			wpf_log( 'notice', 0, 'Unable to parse FooEvents date: ' . $date_string . ' with WordPress date format: ' . $wp_date_format . '. Using original value.' );
			return $date_string;
		}

		// Return in Y-m-d format for CRM integration.
		...
		foreach ( $this->get_attendees_from_order( $order ) as $attendee ) {
			...
			if ( ! isset( $customer_data['event_name'] ) ) {
				...
				$event_fields = array(
					...
					'event_start_date' => $this->format_event_date( get_post_meta( $product_id, 'WooCommerceEventsDate', true ) ),
	...
	public function add_attendee_data( $ticket_id ) {
		...
		if ( empty( $settings ) || empty( $settings['add_attendees'] ) ) {
			wpf_log( 'info', 0, 'Skipping FooEvents attendee #' . $ticket_id . ' because Add Attendees is disabled on the product.' );
			return;
		}
		...
		$event_fields = array(
			...
			'event_name'       => get_the_title( $product_id ),
			'event_sku'        => get_post_meta( $product_id, '_sku', true ),
			'event_start_date' => $this->format_event_date( $start_date ),
			'event_start_time' => $hour . ':' . $minutes . ' ' . $period,
			'event_venue_name' => get_post_meta( $product_id, 'WooCommerceEventsLocation', true ),
			'zoom_meeting_id'  => get_post_meta( $product_id, 'WooCommerceEventsZoomWebinar', true ),
			'zoom_join_url'    => get_post_meta( $product_id, 'wp_fusion_zoom_join_url', true ),
		...
		if ( $order && $update_data['user_email'] === $order->get_billing_email() && ! isset( $_GET['order_action'] ) ) {
			...
			// Only skip if the billing email also matches the user who placed the order.
			$user_id = $order->get_user_id();

			if ( $user_id ) {
				$user = get_userdata( $user_id );
				if ( $user && $user->user_email === $order->get_billing_email() ) {
					return; // Billing email matches both attendee and logged-in user, already processed.
				}
			}
		}
	...
	/**
	 * Sync ticket number to CRM after it's generated.
	 *
	 * FooEvents saves WooCommerceEventsTicketNumber and WooCommerceEventsTicketNumberFormatted
	 * after the fooevents_create_ticket hook fires, so we catch them here.
	 *
	 * @since 3.46.14
	 *
	 ...
	 */
	public function sync_ticket_number( $meta_id, $ticket_id, $meta_key, $meta_value ) {

		if ( 'event_magic_tickets' !== get_post_type( $ticket_id ) ) {
			return;
		}

		if ( 'WooCommerceEventsTicketNumber' !== $meta_key && 'WooCommerceEventsTicketNumberFormatted' !== $meta_key ) {
			return;
		}

		if ( ! wpf_is_field_active( $meta_key ) ) {
			return;
		}
		...
		wpf_log( 'info', 0, 'Syncing FooEvents ticket #' . $ticket_id . ' number to contact #' . $contact_id, array( 'meta_array' => $update_data ) );
	...
	public function add_meta_fields( $meta_fields ) {
		...
		$meta_fields['WooCommerceEventsTicketID'] = array(
			'label'  => 'Ticket ID',
			'type'   => 'text',
			'group'  => 'fooevents_attendee',
			'pseudo' => true,
		...
		$meta_fields['WooCommerceEventsTicketNumber'] = array(
			'label'  => 'Ticket Number',
			'type'   => 'int',
			'group'  => 'fooevents_attendee',
			'pseudo' => true,
		...
		$meta_fields['WooCommerceEventsTicketNumberFormatted'] = array(
			'label'  => 'Ticket Number (Formatted)',
			'type'   => 'text',
			'group'  => 'fooevents_attendee',
			'pseudo' => true,
		);

		$meta_fields['WooCommerceEventsTicketHash'] = array(
			'label'  => 'Ticket Hash',
			'type'   => 'text',
			'group'  => 'fooevents_attendee',
			'pseudo' => true,
