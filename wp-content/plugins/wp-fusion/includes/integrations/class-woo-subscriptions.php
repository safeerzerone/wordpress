...
class WPF_Woo_Subscriptions extends WPF_Integrations_Base {
	 ...
	 * Also manages global subscription tags based on user's overall subscription status.
	 *
	 * @since 3.46.4 Added global subscription tags support.
	 *
	 * @access public
	 * @param WC_Subscription $subscription The subscription object.
	 * @param string|false    $status       The subscription status. If false, will get from subscription.
	 * @return void
	 */
	public function apply_tags_for_subscription_status( $subscription, $status = false ) {
		...
		// Handle global subscription tags.
		$global_tags = wpf_get_option( 'woo_subscriptions_active_tags', array() );
		...
		if ( ! empty( $global_tags ) ) {
			...
			foreach ( $active_subscriptions as $active_subscription ) {
				if ( 'active' === $active_subscription->get_status() ) {
					$has_active = true;
					break;
				}
			}
			...
			if ( $has_active ) {
				// Apply global tags if user has any active subscription.
				$apply_tags = array_merge( $apply_tags, $global_tags );
			} elseif ( wpf_get_option( 'woo_subscriptions_remove_tags' ) ) {
				// Remove global tags if enabled and user has no active subscriptions.
				$remove_tags = array_merge( $remove_tags, $global_tags );
			}
		}
	...
	public function sync_subscription_fields( $subscription ) {
		...
		$update_data = array(
			...
			'sub_next_payment_value' => $subscription->get_total(),
		);
		...
		foreach ( $subscription->get_items() as $line_item ) {
			...
			$update_data[ 'sub_next_payment_value_' . $product_id ] = $subscription->get_total();
	...
	public function panel_content( $post_id ) {
		...
		$wpf_settings = get_post_meta( $post_id, 'wpf-settings-woo', true );

		if ( is_array( $wpf_settings ) && ! empty( $wpf_settings ) ) {
			$settings = wp_parse_args( $wpf_settings, $settings );
		}
	...
	private function get_product_subscription_crm_fields() {
		return array(
			...
			'sub_next_payment_value' => array(
				'name' => __( 'Next Payment Value', 'wp-fusion' ),
				'type' => 'text',
			),
		);
	}
	...
	/**
	 * Registers global settings for WooCommerce Subscriptions.
	 *
	 * @since 3.46.4
	 *
	...
	public function register_settings( $settings, $options ) {
		...
		$settings['woo_subscriptions_header'] = array(
			'title'   => __( 'WooCommerce Subscriptions - Global Tags', 'wp-fusion' ),
			'url'     => 'https://wpfusion.com/documentation/ecommerce/woocommerce-subscriptions/',
			'type'    => 'heading',
			'section' => 'integrations',
		);

		$settings['woo_subscriptions_active_tags'] = array(
			'title'   => __( 'Apply Tags - Any Active Subscription', 'wp-fusion' ),
			'desc'    => __( 'These tags will be applied when a user has any active subscription.', 'wp-fusion' ),
			'std'     => array(),
			'type'    => 'assign_tags',
			'section' => 'integrations',
		);

		$settings['woo_subscriptions_remove_tags'] = array(
			'title'   => __( 'Remove Tags When Inactive', 'wp-fusion' ),
			'desc'    => __( 'Remove the tags above when the user has no active subscriptions.', 'wp-fusion' ),
			'type'    => 'checkbox',
			'section' => 'integrations',
		);
