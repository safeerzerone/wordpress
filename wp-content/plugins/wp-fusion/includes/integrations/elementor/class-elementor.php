...
class WPF_Elementor extends WPF_Integrations_Base {
	...
	/**
	 * Migrates legacy tag IDs in Elementor's JSON data.
	 *
	 * Walks _elementor_data and _elementor_popup_display_settings
	 * postmeta for all posts that use Elementor.
	 *
	 * @since 3.47.8
	 *
	...
	public function migrate_tag_ids( $id_map ) {
		...
		$meta_keys = array( '_elementor_data', '_elementor_popup_display_settings' );
		$updated   = 0;
		...
		foreach ( $meta_keys as $meta_key ) {
			...
			$rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT meta_id, post_id, meta_value FROM {$wpdb->postmeta} WHERE meta_key = %s AND meta_value != ''",
					$meta_key
				)
			);
			...
			foreach ( $rows as $row ) {
				...
				if ( '_elementor_data' === $meta_key && is_string( $data ) ) {
				...
				list( $new_data, $changed ) = $this->replace_ids_in_nested_data( $data, $target_keys, $id_map );

				if ( $changed ) {
					$save_value = $was_json ? wp_json_encode( $new_data ) : $new_data;
					update_post_meta( absint( $row->post_id ), $meta_key, $save_value );
					++$updated;
				}
			}
		}
	...
	/**
	 * Recursively replaces legacy tag IDs in nested Elementor data.
	 *
	 * @since 3.47.8
	 *
	 ...
	 */
	private function replace_ids_in_nested_data( $data, $target_keys, $id_map ) {
		...
		foreach ( $data as $key => $value ) {
			if ( in_array( $key, $target_keys, true ) && is_array( $value ) ) {
				...
				list( $new_value, $nested_changed ) = $this->replace_ids_in_nested_data( $value, $target_keys, $id_map );
	...
	/**
	 * Returns a declarative description of Elementor's tag storage locations.
	 *
	 * Elementor stores WP Fusion tag settings inside JSON blobs in _elementor_data
	 * and _elementor_popup_display_settings. The values for these keys are empty
	 * arrays because the actual tag keys are nested recursively — callers should
	 * use WPF_Admin_Tag_Migration::get_elementor_tag_keys() to identify them.
	 *
	 * @since 3.47.8
	 *
	 * @return array<string, array<string>> Meta key => array of direct sub-keys (empty for JSON blobs).
	 */
	public function get_tag_settings() {

		return array(
			'_elementor_data'                   => array(),
			'_elementor_popup_display_settings' => array(),
		);
	}


	/**
	 * Returns all WP Fusion tag IDs currently configured in Elementor widgets.
	 *
	 * Scans _elementor_data and _elementor_popup_display_settings postmeta for
	 * any keys matching the Elementor tag key list and collects every tag ID
	 * found. Results are deduplicated.
	 *
	 * @since 3.47.8
	 *
	 * @return array<int|string> Flat array of unique tag IDs.
	 */
	public function get_configured_tag_ids() {

		global $wpdb;

		$tag_keys   = WPF_Admin_Tag_Migration::get_elementor_tag_keys();
		$meta_keys  = array( '_elementor_data', '_elementor_popup_display_settings' );
		$found_tags = array();
		...
		$rows = $wpdb->get_results( $wpdb->prepare( "SELECT meta_key, meta_value FROM {$wpdb->postmeta} WHERE meta_key IN ({$placeholders}) AND meta_value != ''", $meta_keys ) );

		foreach ( $rows as $row ) {
			$data = maybe_unserialize( $row->meta_value );

			if ( '_elementor_data' === $row->meta_key && is_string( $data ) ) {
				$decoded = json_decode( $data, true );
			...
			if ( is_array( $data ) ) {
				$found_tags = array_merge( $found_tags, $this->collect_tag_ids_from_nested_data( $data, $tag_keys ) );
			}
		}

		return array_values( array_unique( $found_tags ) );
	}


	/**
	 * Recursively collects tag IDs from nested arrays by matching known keys.
	 *
	 * @since 3.47.8
	 *
	 * @param array $data        Nested data array.
	 * @param array $target_keys Keys whose values should be collected.
	 * @return array<int|string> Flat list of tag IDs found.
	 */
	private function collect_tag_ids_from_nested_data( $data, $target_keys ) {
		...
		foreach ( $data as $key => $value ) {
			if ( in_array( $key, $target_keys, true ) && is_array( $value ) ) {
				...
				$found = array_merge( $found, $this->collect_tag_ids_from_nested_data( $value, $target_keys ) );
			}
		}
