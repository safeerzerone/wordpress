...
class WPF_EDD extends WPF_Integrations_Base {
	...
	public function init() {
		...
		// EDD Tabbed Interface Integration
		if ( version_compare( EDD_VERSION, '3.3.6', '>=' ) ) {
			add_filter( 'edd_download_details_sections', array( $this, 'add_download_details_section' ), 10, 1 );
		} else {
			// Fallback to old metabox for older EDD versions
		...
		if ( version_compare( EDD_VERSION, '3.0.0', '>' ) ) {
			...
			add_action( 'template_redirect', array( $this, 'maybe_auto_apply_discounts' ) );
		}
		...
		add_action( 'wpf_batch_edd_refunds', array( $this, 'batch_step_refunds' ) );
	}
	...
	public function includes() {
		// Only load the download section class if EDD supports the new tabbed interface.
		if ( class_exists( '\EDD\Admin\Downloads\Editor\Section' ) ) {
			require_once __DIR__ . '/class-edd-download-section.php';
		}
	}
	...
	public function maybe_auto_apply_discounts( $user_tags = false ) {
		...
		// Get cart product IDs for product requirement checking.
		// Cast to integers for consistent comparison.
		$cart_product_ids = array_map( 'absint', wp_list_pluck( $cart_items, 'id' ) );
		...
		foreach ( $discounts as $discount_id ) {
			...
			if ( ! empty( array_intersect( $user_tags, $settings['auto_apply_tags'] ) ) ) {
				...
				// Check product requirements before applying.
				// This ensures the discount is only auto-applied when the required products are in the cart.
				...
				if ( ! empty( $product_reqs ) ) {
					...
					if ( 'all' === $condition ) {
						...
						if ( ! empty( array_diff( $product_reqs, $cart_product_ids ) ) ) {
							continue;
						}
					} else {
						// Any required product must be in cart (default "any" condition).
						if ( empty( array_intersect( $product_reqs, $cart_product_ids ) ) ) {
							continue;
						}
					}
				}

				// Let EDD validate the discount (checks expiry, usage limits, min cart amount, etc.).
				if ( ! edd_is_discount_valid( $discount->get_code(), wpf_get_current_user_id(), false ) ) {
					continue;
				}
	...
	/**
	 * Adds WP Fusion section to EDD download details tabs
	 *
	 * @since 3.46.7
	 *
	 * @param array $sections The existing sections.
	 * @return array Modified sections with WP Fusion section added.
	 */
	public function add_download_details_section( $sections ) {

		$sections['wp_fusion'] = 'WPF_EDD_Download_Section';
