...
class WPF_If_So extends WPF_Integrations_Base {
	...
	public function add_new_rule( $new_version_rules, $group_item ) {
		$tags = isset( $group_item['wpfTags'] ) ? (array) $group_item['wpfTags'] : array();
		...
		$new_version_rules['wpf-tags'] = ! empty( $tags ) ? $tags : null;
	...
	public function add_condition( $rule ) {
		...
		<?php
		echo ( isset( $rule['trigger_type'] ) && 'wpfTags' === $rule['trigger_type'] ) ? 'SELECTED' : '';
		if ( function_exists( 'generateDataAttributes' ) ) {
		...
		>
			...
			printf( esc_html__( '%s Tags (any)', 'wp-fusion' ), esc_html( wp_fusion()->crm->name ) );
			?>
		</option>
		<?php
	}
	...
	public function condition_content( $rule, $current_version_index ) {
		...
		$selected_tags = array_map( 'strval', $rule['wpf-tags'] );
		...
		<div class="ifso-form-group wpf_tags">
				...
				<select name="repeater[<?php echo esc_attr( $current_version_index ); ?>][wpfTags][]" multiple class="wpf_ifso_select form-control referrer-custom <?php echo ( ! empty( $rule['trigger_type'] ) && 'wpfTags' === $rule['trigger_type'] ) ? 'show-selection' : ''; ?>" data-field="wpf-tags">
						<?php
						$available_tags = wp_fusion()->settings->get_available_tags_flat( true, false );
						foreach ( $available_tags as $id => $tag ) {
							$is_selected = in_array( (string) $id, $selected_tags, true );
							echo '<option value="' . esc_attr( $id ) . '" ' . selected( $is_selected, true, false ) . '>' . esc_html( $tag ) . '</option>';
						}
						?>
				</select>
		</div>
...
SCR;
	 ...
	 * @since  3.42.13
	 *
	 * @param  array $models The existing UI models for Gutenberg conditions.
	 * @return array         The modified UI models with WP Fusion trigger added.
	 */
	public function add_trigger_in_gutenberg( $models ) {
		...
		$fields['wpf-tags'] = new \ConditionUIElement( 'wpf-tags', '', 'select', true, $options );
		$model              = (object) array(
			'fields' => $fields,
			'name'   => $name,
		);
...
if ( ! class_exists( 'ConditionUIElement' ) ) {
	/**
	 * Condition UI Element helper class.
	 *
	 * @since 3.38.0
	 */
	class ConditionUIElement {
		 ...
		 * @param string      $prettyName       Pretty display name.
		 * @param string      $type             Field type.
		 * @param bool        $required         Whether field is required.
		 * @param array|null  $options          Field options.
		 * @param bool        $is_switcher      Whether field is a switcher.
		 * @param string|null $subgroup         Subgroup name.
		 * @param string      $extraClasses     Additional CSS classes.
		 * @param string|null $symbol           Field symbol.
		 * @param bool        $autocompleteOpts Whether to enable autocomplete.
		 */
		public function __construct( $name, $prettyName, $type, $required = false, $options = null, $is_switcher = false, $subgroup = null, $extraClasses = '', $symbol = null, $autocompleteOpts = false ) {
			$this->name             = $name;
...
if ( IFSO_PLUGIN_SERVICES_BASE_DIR ) {
	...
	/**
	 * WP Fusion Tags Trigger for If-So.
	 *
	 * @since 3.38.0
	 */
	class WPF_Tags_Trigger extends \IfSo\PublicFace\Services\TriggersService\Triggers\TriggerBase {
		...
		public function can_handle( $trigger_data ) {
			...
			if ( 'true' === $rule['freeze-mode'] ) {
				return false;
			}
			if ( $this->trigger_name !== $rule['trigger_type'] ) {
				return false;
			}
