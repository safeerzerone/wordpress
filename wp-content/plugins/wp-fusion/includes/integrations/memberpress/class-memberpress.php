...
class WPF_MemberPress extends WPF_Integrations_Base {
	...
	public function add_to_membership( $user_id, $user_tags ) {
		...
		// Group products by their group ID to handle upgrade paths.
		$grouped_products     = array();
		$non_grouped_products = array();
		...
		foreach ( $linked_products as $product_id ) {
			...
			// Only process products that have tags present for this user.
			...
			if ( ! empty( $product->group_id ) ) {
				...
				// Only handle groups that are upgrade paths.
				if ( $group->is_upgrade_path ) {
					if ( ! isset( $grouped_products[ $product->group_id ] ) ) {
						$grouped_products[ $product->group_id ] = array();
					}
					$grouped_products[ $product->group_id ][] = array(
					...
					// Non-upgrade path groups are treated as regular products.
					$non_grouped_products[] = array(
						'product_id' => $product_id,
						'tag_id'     => $tag_id,
					);
		...
		// Process grouped products - only enroll in the highest tier for each group.
		foreach ( $grouped_products as $group_id => $products ) {
			// Sort by group_order descending to get highest tier first.
			usort(
				$products,
				function ( $a, $b ) {
			...
			// Get the highest tier product.
			$highest_tier = $products[0];

			// Check if user is already subscribed to this product.
			if ( ! $mepr_user->is_already_subscribed_to( $highest_tier['product_id'] ) ) {

				// Check if user is subscribed to any other product in this group.
				$current_group_subscription = null;
				foreach ( $products as $product_data ) {
					if ( $mepr_user->is_already_subscribed_to( $product_data['product_id'] ) ) {
						$current_group_subscription = $product_data;
						break;
					}
				}

				// If user has a lower tier subscription, we'll let the upgrade process handle it.
				if ( $current_group_subscription && $current_group_subscription['group_order'] < $highest_tier['group_order'] ) {
					// The user has a lower tier, proceed with upgrade.
					$this->enroll_user_in_membership( $user_id, $highest_tier['product_id'], $highest_tier['tag_id'] );
				} elseif ( ! $current_group_subscription ) {
					// User has no subscription in this group, enroll them.
					$this->enroll_user_in_membership( $user_id, $highest_tier['product_id'], $highest_tier['tag_id'] );
				} else {
					// User already has this tier or a higher tier.
					wpf_log( 'info', $user_id, 'Skipping enrollment in <a href="' . admin_url( 'post.php?post=' . $highest_tier['product_id'] . '&action=edit' ) . '" target="_blank">' . get_the_title( $highest_tier['product_id'] ) . '</a> because user already has an equal or higher tier membership in the group.' );
				}
			}
		}

		// Process non-grouped products normally.
		foreach ( $non_grouped_products as $product_data ) {
			if ( ! $mepr_user->is_already_subscribed_to( $product_data['product_id'] ) ) {
				$this->enroll_user_in_membership( $user_id, $product_data['product_id'], $product_data['tag_id'] );
			}
		}

		// Handle unenrollment for products whose tags were removed.
		foreach ( $linked_products as $product_id ) {
			...
			if ( ! in_array( $tag_id, $user_tags ) && $mepr_user->is_already_subscribed_to( $product_id ) ) {
				...
				foreach ( $transactions as $transaction ) {
					...
					if ( $transaction->product_id == $product_id && ( 'free' == $transaction->gateway || 'manual' == $transaction->gateway || 'unknown' == $transaction->gateway ) ) {
				...
				if ( ! $did_it ) {
					wpf_log( 'notice', $user_id, 'Automated unenrollment failed. For security reasons WP Fusion can only unenroll users from transactions created with the "free", "manual", or "unknown" gateways.' );
				}
			}
		}
	}
	...
	/**
	 * Enroll a user in a membership.
	 *
	 * @since 3.46.1
	 *
	 * @param int $user_id    User ID.
	 * @param int $product_id Product ID.
	 * @param int $tag_id     Tag ID that triggered the enrollment.
	 */
	private function enroll_user_in_membership( $user_id, $product_id, $tag_id ) {
		...
		if ( 'lifetime' !== $product->period_type ) { // A free recurring subscription? Nope - let's make it lifetime for free here folks.
		...
		$free_gateway = new MeprBaseStaticGateway( 'free', __( 'Free', 'wp-fusion' ), __( 'Free', 'wp-fusion' ) );
		...
		MeprHooks::do_action( 'mepr-signup', $txn ); // This lets the Corporate Accounts addon know there's been a new signup.

		add_action( 'mepr-signup', array( $this->transactions, 'transaction_complete' ) );

		add_action( 'mepr-txn-status-complete', array( $this->transactions, 'transaction_complete' ) );
	}
