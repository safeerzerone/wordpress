...
class WPF_MemberPress_Courses {
	...
	public function __construct() {
		...
		add_filter( 'mpcs_user_can_view_course', array( $this, 'user_can_view_course' ), 10, 2 );
	...
	/**
	 * Allow auto-login users to view courses.
	 *
	 * @since 3.46.6
	 *
	 * @param bool   $can_view Whether the user can view the course.
	 * @param object $course The course object.
	 * @return bool Whether the user can view the course.
	 */
	public function user_can_view_course( $can_view, $course ) {
	...
	public function quiz_progress( $progress ) {
		...
		// Check if MemberPress Quizzes Attempt class is available
		if ( ! class_exists( '\memberpress\quizzes\models\Attempt' ) ) {
			wpf_log( 'info', 0, 'MemberPress Quizzes Attempt model not found. Skipping quiz progress tracking.', array( 'source' => 'memberpress-courses' ) );
			return;
		}

		$attempt = \memberpress\quizzes\models\Attempt::get_one(
			array(
				'quiz_id' => $quiz_id,
		...
		if ( ! $attempt ) {
			wpf_log( 'info', $attrs['user_id'], 'No quiz attempt found for user ID ' . $attrs['user_id'] . ' and quiz ID ' . $quiz_id, array( 'source' => 'memberpress-courses' ) );
			return;
		}
