...
class WPF_MemberPress_Transactions {
	...
	public function transaction_complete( $event ) {
		...
		if ( ! did_action( 'wpf_user_register_start' ) ) {
		...
		if ( ! in_array( $txn->user_id, $this->pushed_meta_ids, true ) ) {
			...
			if ( ! wpf_get_contact_id( $txn->user_id ) && ! did_action( 'wpf_user_register_start' ) ) {
				// Don't create a new contact if one is already being created via the user_register action.
	...
	/**
	 * Syncs the membership level name based on the user's current active memberships.
	 *
	 * This is needed when a membership expires and the user is automatically downgraded
	 * to a free tier, since MemberPress doesn't create a new transaction for the downgrade.
	 *
	 * @since 3.47.1
	 *
	 * @param int $user_id The user ID.
	 */
	public function sync_active_membership_level( $user_id ) {

		$product_ids = wp_fusion()->integrations->memberpress->get_product_memberships( $user_id );

		if ( empty( $product_ids ) || empty( $product_ids[0] ) ) {
			return; // No active memberships.
		}

		// Get the first active membership (primary membership).
		$membership_level = html_entity_decode( get_the_title( $product_ids[0] ) );

		$update_data = array(
			'mepr_membership_level' => $membership_level,
		);
		...
		wpf_log(
			'info',
			$user_id,
			'Updated membership level to <strong>' . esc_html( $membership_level ) . '</strong> based on current active membership.',
			array( 'source' => 'memberpress' )
		);
	}
	...
	public function transaction_expired( $txn ) {
		...
		if ( false === $group ) {
			...
			$this->sync_active_membership_level( $txn->user_id );
