...
class WPF_UM extends WPF_Integrations_Base {
	...
	public function tags_modified( $user_id, $user_tags ) {
		...
		if ( ! empty( $deactivation_tag ) && ! empty( $deactivation_tag[0] ) ) {
			...
			if ( in_array( $deactivation_tag[0], $user_tags ) ) {
				...
				UM()->common()->users()->deactivate( $user_id );

			} elseif ( ! in_array( $deactivation_tag[0], $user_tags ) && UM()->common()->users()->can_be_reactivated( $user_id ) ) {

				wpf_log( 'notice', $user_id, 'User\'s account was reactivated by the removal of the deactivation tag <strong>' . wpf_get_tag_label( $deactivation_tag[0] ) . '</strong>' );
				UM()->common()->users()->reactivate( $user_id );
	...
	public function metabox_scripts() {
		...
		wp_enqueue_script( 'wpf-admin', WPF_DIR_URL . 'assets/js/wpf-admin.js', array( 'jquery', 'select4', 'wp-i18n' ), WP_FUSION_VERSION, true );
