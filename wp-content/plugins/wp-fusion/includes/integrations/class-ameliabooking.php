...
class WPF_AmeliaBooking extends WPF_Integrations_Base {
	...
	public function init() {
		...
		// Sync bookings created via redirect payment gateways (Mollie, Barion, etc.).
		add_action( 'amelia_after_appointment_booking_saved', array( $this, 'process_appointment_booking_saved' ), 10, 3 );
		add_action( 'amelia_after_event_booking_saved', array( $this, 'process_event_booking_saved' ), 10, 2 );
	...
	public function process_booking( $data ) {
		...
		if ( ! empty( $booking['customFields'] ) ) {
			$custom_fields = is_string( $booking['customFields'] ) ? json_decode( $booking['customFields'], true ) : $booking['customFields'];
			...
			if ( is_array( $custom_fields ) ) {
				foreach ( $custom_fields as $field_id => $field ) {
					...
					if ( empty( $field['id'] ) && is_numeric( $field_id ) ) {
						$field['id'] = $field_id;
					}

					$key = $this->get_custom_field_key( $field );

					if ( empty( $key ) ) {
						continue;
					}

					$customer_data[ $key ] = $this->get_custom_field_value( $field );
				}
			}
		}
	...
	/**
	 * Process appointment booking saved.
	 *
	 * Handles appointments created via redirect payment gateways like Mollie.
	 *
	 * @since 3.47.4
	 *
	 ...
	 */
	public function process_appointment_booking_saved( $booking, $service, $appointment ) {

		// Skip if this is not from a redirect payment gateway.
		// The amelia_after_booking_added hook will handle standard bookings.
		if ( empty( $booking['status'] ) || 'pending' !== $booking['status'] ) {
			return;
		}

		$this->process_saved_booking( 'appointment', $booking, $service, $appointment );
	}


	/**
	 * Process event booking saved.
	 *
	 * Handles events created via redirect payment gateways like Mollie.
	 *
	 * @since 3.47.4
	 *
	 * @param array $booking The booking data.
	 ...
	 */
	public function process_event_booking_saved( $booking, $event ) {
		...
		$this->process_saved_booking( 'event', $booking, $event );
	}


	/**
	 * Process saved booking from payment gateway hooks.
	 *
	 * @since 3.47.4
	 *
	 * @param string     $type        The booking type (appointment or event).
	 * @param array      $booking     The booking data.
	 * @param array      $bookable    The service or event data.
	 * @param array|null $appointment The appointment data (only for appointments).
	 */
	private function process_saved_booking( $type, $booking, $bookable, $appointment = null ) {
		...
		// Add type-specific data.
		if ( 'appointment' === $type ) {
			$customer_data['appointment_date_time'] = $appointment['bookingStart'];
			$customer_data['service_name']          = $bookable['name'];
			$settings                               = is_string( $bookable['settings'] ) ? json_decode( $bookable['settings'], true ) : $bookable['settings'];
		} else {
			$customer_data['event_date_time'] = ! empty( $bookable['periods'][0]['periodStart'] ) ? $bookable['periods'][0]['periodStart'] : '';
			$customer_data['event_name']      = $bookable['name'];
			$settings                         = is_string( $bookable['settings'] ) ? json_decode( $bookable['settings'], true ) : $bookable['settings'];
		}
		...
		if ( ! empty( $settings['apply_tags'] ) ) {
			if ( empty( $user ) ) {
				// Guest checkout.
				wpf_log( 'info', 0, "Applying tags for guest {$type} booking (redirect payment) to contact #{$contact_id}: ", array( 'tag_array' => $settings['apply_tags'] ) );
				wp_fusion()->crm->apply_tags( $settings['apply_tags'], $contact_id );
			} else {
				// Registered users.
				wp_fusion()->user->apply_tags( $settings['apply_tags'], $user->ID );
	 ...
	 */
	private function get_custom_field_key( $field ) {
		...
		if ( ! empty( $field['label'] ) ) {
			...
			$labels = $this->get_custom_field_labels();
			$label  = ! empty( $labels[ $field['id'] ] ) ? $labels[ $field['id'] ] : '';
		}
	...
	/**
	 * Normalize Amelia custom field values.
	 *
	 * @since 3.47.6
	 *
	 * @param array $field The custom field data.
			...
			if ( array_key_exists( 'value', $value ) ) {
				...
				foreach ( $value as $item ) {
					if ( is_array( $item ) ) {
						...
						} elseif ( array_key_exists( 'label', $item ) ) {
				...
				$value = implode( ', ', array_map( 'strval', $normalized ) );
			}
		}
	...
	/**
	 * Get Amelia custom field labels keyed by ID.
	 *
	 * @since 3.47.6
	 *
	 * @return array The custom field labels keyed by ID.
	 */
	private function get_custom_field_labels() {
		...
		$table = $wpdb->prefix . 'amelia_custom_fields';
		// phpcs:ignore -- Table name is safe.
		$fields = $wpdb->get_results( "SELECT id, label FROM {$table}", ARRAY_A );
		$labels = array();
