<?php
/**
 * WP Fusion - LifterLMS Integration
 *
 * @package   WP Fusion
...
/**
 * Handles integration with LifterLMS learning management system.
 *
 * @since 2.7.1
 */
class WPF_LifterLMS extends WPF_Integrations_Base {
	...
	public function init() {
		...
		// Access plans.
		add_action( 'lifterlms_order_status_completed', array( $this, 'complete_order' ) );
		add_action( 'lifterlms_order_status_active', array( $this, 'complete_order' ) );
		add_action( 'lifterlms_order_status_pending', array( $this, 'transition_order_status' ), 10, 3 );
		add_action( 'lifterlms_order_status_on-hold', array( $this, 'transition_order_status' ), 10, 3 );
		add_action( 'lifterlms_order_status_cancelled', array( $this, 'transition_order_status' ), 10, 3 );
		add_action( 'lifterlms_order_status_refunded', array( $this, 'transition_order_status' ), 10, 3 );
		add_action( 'lifterlms_order_status_failed', array( $this, 'transition_order_status' ), 10, 3 );
	...
	/**
	 * Register Settings
	 * Registers additional LifterLMS settings.
	 *
	 * @since 3.46.9
	 *
	...
	public function register_settings( $settings, $options ) {
		...
		$settings['lifterlms_header_orders'] = array(
			'title'   => __( 'LifterLMS Order Statuses', 'wp-fusion' ),
			'type'    => 'heading',
			'desc'    => __( '<p>The settings here let you apply tags to a contact when an order status is changed in LifterLMS.</p><p>This is useful for tracking changes in status, for example when orders are marked pending or failed.</p>', 'wp-fusion' ),
			'section' => 'integrations',
		);

		$statuses = array(
			'pending'   => __( 'Pending', 'wp-fusion' ),
			'on-hold'   => __( 'On Hold', 'wp-fusion' ),
			'cancelled' => __( 'Cancelled', 'wp-fusion' ),
			'refunded'  => __( 'Refunded', 'wp-fusion' ),
			'failed'    => __( 'Failed', 'wp-fusion' ),
		);

		foreach ( $statuses as $status => $label ) {
			$settings[ 'lifterlms_tags_' . $status ] = array(
		...
		$fields[] = array(
			...
			'fields' => array(
				...
				array(
					...
					/* translators: %1$s and %2$s: CRM name */
	...
	/**
	 * Adds WPF settings to LLMS course/lesson meta box
	 *
	 * @since 2.7.1
	 *
	 * @param  array $fields The existing fields array.
	...
	public function course_lesson_metabox( $fields ) {
		...
		$fields['wpf']['fields'][] = array(
			...
			/* translators: %s: post type (course/lesson) */
	 ...
	 * @since 2.7.1
	 *
	 * @param LLMS_Access_Plan|null $plan  The access plan object.
	 * @param int                   $id    The plan ID.
	 * @param int                   $order The plan order.
	 */
	public function access_plan_settings( $plan, $id, $order ) {
		...
		<h4><?php esc_html_e( 'WP Fusion Settings', 'wp-fusion' ); ?></h4>

		<div class="llms-plan-row-wpf">

			<div class="llms-metabox-field d-1of2">

				<label for="_llms_plans[<?php echo esc_attr( $order ); ?>][apply_tags]">
					<?php esc_html_e( 'Apply Tags', 'wp-fusion' ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'These tags will be applied when a user purchases this access plan.', 'wp-fusion' ); ?></span>
					<span class="tip--top-right" data-tip="<?php esc_attr_e( 'These tags will be applied when a user purchases this access plan.', 'wp-fusion' ); ?>">
		...
		<div class="llms-plan-row-wpf">
			...
			<div class="llms-metabox-field d-1of2">
				...
				<label for="_llms_plans[<?php echo esc_attr( $order ); ?>][allow_tags]">
					<?php esc_html_e( 'Required Tags', 'wp-fusion' ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'User must have these tags to see this access plan.', 'wp-fusion' ); ?></span>
					<span class="tip--top-right" data-tip="<?php esc_attr_e( 'User must have these tags to see this access plan.', 'wp-fusion' ); ?>">
						<i class="fa fa-question-circle"></i>
					</span>
				</label>
				<?php
			...
			/**
			 * Allows add-on plugins to add additional fields to the WP Fusion access plan settings.
			 *
			 * @since 3.47.4.1
			 *
			 ...
			 * @param int              $order The plan order.
			 */
			do_action( 'wpf_lifterlms_access_plan_fields', $plan, $id, $order );
	 ...
	 *
	 * @param LLMS_Access_Plan $plan          The access plan object.
	 * @param array            $raw_plan_data Raw plan data from form.
	 * @param LLMS_Metabox     $metabox       The metabox object.
	 * @return void
	 */
	...
	public function save_meta_box_data( $post_id ) {
		...
		} elseif ( 'llms_voucher' === $post_type ) {

			// Vouchers.

			if ( ! empty( $_POST['wpf_settings_llms_voucher'] ) ) {
				$voucher_settings = array();
				foreach ( wp_unslash( $_POST['wpf_settings_llms_voucher'] ) as $key => $value ) {
					if ( is_array( $value ) ) {
						$voucher_settings[ sanitize_key( $key ) ] = array_map( 'sanitize_text_field', $value );
					} else {
						$voucher_settings[ sanitize_key( $key ) ] = sanitize_text_field( $value );
					}
				}
				update_post_meta( $post_id, 'wpf_settings_llms_voucher', $voucher_settings );
			} else {
		...
		if ( ! empty( $_POST['wpf-settings-llms-plan'] ) ) {
			...
			$plan_settings = wpf_clean( wp_unslash( $_POST['wpf-settings-llms-plan'] ) );

			foreach ( $plan_settings as $plan_id => $setting ) {
	...
	public function complete_order( $order ) {
		...
		// Remove tags from non-active order statuses when order becomes active/completed.
		$non_active_statuses = array( 'pending', 'on-hold', 'cancelled', 'refunded', 'failed' );
		$remove_tags         = array();

		foreach ( $non_active_statuses as $status ) {
			$status_tags = wpf_get_option( 'lifterlms_tags_' . $status );
			if ( ! empty( $status_tags ) ) {
				$remove_tags = array_merge( $remove_tags, $status_tags );
			}
		}
		...
		if ( ! empty( $remove_tags ) ) {
			...
			wp_fusion()->user->remove_tags( $remove_tags, $order->get( 'user_id' ) );
		}

		$settings = get_post_meta( $order->get( 'plan_id' ), 'wpf-settings-llms-plan', true );

		if ( ! empty( $settings ) && ! empty( $settings['apply_tags'] ) ) {
			wp_fusion()->user->apply_tags( $settings['apply_tags'], $order->get( 'user_id' ) );
		}

		update_post_meta( $order->get( 'id' ), 'wpf_complete', current_time( 'Y-m-d H:i:s' ) );
	}
	 ...
	 * specified in the LifterLMS Order Statuses settings.
	 ...
	 */
	public function transition_order_status( $order, $old_status, $new_status ) {

		// We don't need to check apply or remove tags if the status hasn't changed.
		if ( $new_status === $old_status || empty( $new_status ) || empty( $old_status ) ) {
			return;
		}

		$apply_tags = wpf_get_option( 'lifterlms_tags_' . $new_status );

		if ( ! empty( $apply_tags ) ) {
			// Apply tags.
			wp_fusion()->user->apply_tags( $apply_tags, $order->get( 'user_id' ) );
		}
	}
	 ...
	 * @since 2.7.1
	 *
	 * @param array $triggers The existing trigger options.
	 ...
	 * @since 2.7.1
	 *
	 * @param array $fields The existing engagement fields.
	 * @return array Fields
	 */
	...
	public function save_engagement_data( $post_id ) {
		...
		if ( isset( $_POST['post_type'] ) && 'llms_engagement' === sanitize_text_field( wp_unslash( $_POST['post_type'] ) ) && isset( $_POST['_llms_engagement_trigger_tag'] ) ) {

			update_post_meta( $post_id, '_llms_engagement_trigger_tag', sanitize_text_field( wp_unslash( $_POST['_llms_engagement_trigger_tag'] ) ) );
	 ...
	 *
	 * @param int   $user_id   The user ID.
	 * @param array $user_tags Array of user's current tags.
	 * @return void
	 */
	public function update_engagements( $user_id, $user_tags ) {
		...
		foreach ( $engagements as $engagement_id ) {
			...
			if ( in_array( $tag, $user_tags, true ) ) {
				...
				if ( 'achievement' === $type && ! in_array( $award_id, $student_achievement_ids, true ) ) {
				...
				} elseif ( 'certificate' === $type && ! in_array( $award_id, $student_certificate_ids, true ) ) {
	 ...
	 * @since 2.7.1
	 *
	 * @param WP_Term $term The term object for the track.
	...
	public function course_track_form_fields( $term ) {
		...
		<table class="wpf-settings-table form-table">
			...
			<tbody>
				...
				<tr class="form-field">
					<th style="padding-bottom: 0px;" colspan="2"><h3 style="margin: 0px;"><?php esc_html_e( 'WP Fusion - Course Track Settings', 'wp-fusion' ); ?></h3></th>
				</tr>

				<tr class="form-field">
					<th scope="row" valign="top"><label for="wpf-lock-content"><?php esc_html_e( 'Apply Tags', 'wp-fusion' ); ?></label></th>
					<td style="max-width: 400px;">
						...
						<span class="description"><?php esc_html_e( 'Apply these tags when all courses in this track are marked complete.', 'wp-fusion' ); ?></span>
	...
	public function render_groups_settings_page() {
		...
		<div class="wrap">
			...
			<form id="wpf-llms-groups-settings" action="" method="post">
				...
				<h4><?php esc_html_e( 'Group Tags', 'wp-fusion' ); ?></h4>
				...
				<?php if ( empty( $groups ) ) : ?>
					...
					<strong><?php esc_html_e( 'No groups found.', 'wp-fusion' ); ?></strong>
				...
				<?php else : ?>
					...
					<table class="table table-hover wpf-settings-table" id="wpf-wishlist-levels-table">
						<thead>
						<tr>
							<th><?php esc_html_e( 'Group Name', 'wp-fusion' ); ?></th>
							<th><?php esc_html_e( 'Apply Tags', 'wp-fusion' ); ?></th>
							<th><?php esc_html_e( 'Remove Tags', 'wp-fusion' ); ?></th>
						</tr>
						</thead>
						<tbody>
