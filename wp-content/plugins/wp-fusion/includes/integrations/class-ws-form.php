...
class WPF_WS_Form extends WS_Form_Action {
	...
	public function __construct() {
		...
		// Fix nonce verification for WP Fusion auto-login users.
		add_action( 'init', array( $this, 'fix_auto_login_nonce' ), 1 );
	...
	public function post( $form, &$submit, $config ) {
		...
		foreach ( $this->field_mapping as $field_map ) {
			...
			// Save to update data.
			$update_data[ $wpf_field ] = apply_filters( 'wpf_format_field_value', $wpf_value, $field_type, $wpf_field, $update_data );
		}
	...
	public function parse_value( &$value, &$type, &$email_address ) {
		...
		switch ( $type ) {
			...
			case 'datetime-local':
			case 'date':
				// Convert date strings to Unix timestamps for CRM compatibility.
				...
				// Normalize all date field types to 'date' for CRM compatibility.
				$type = 'date';
				break;
	...
	public function config_meta_keys( $meta_keys = array(), $form_id = 0 ) {
		...
		$wpf_fields        = wpf_get_option( 'crm_fields', array() );
	...
	/**
	 * Fix WS Form nonce verification for auto-login sessions.
	 *
	 * When auto-login session is used, WS Form actions like hooks, webhooks, or redirects
	 * weren't running due to nonce verification failure. The nonce was being generated
	 * before the auto-login session started, so when WS Form tried to verify it, it failed.
	 *
	 * @since 3.46.7
	 */
	public function fix_auto_login_nonce() {

		// Check if this is a WS Form submission.
		if ( ! doing_wpf_auto_login() || ! isset( $_POST['wsf_form_id'] ) || ! isset( $_POST['wsf_nonce'] ) ) {
			return;
		}

		// Regenerate the nonce now that the auto-login session has started.
		$_POST['wsf_nonce'] = wp_create_nonce( 'ws_form_post' );
		wpf_log( 'info', wpf_get_current_user_id(), 'Regenerated WS Form nonce for auto-login session.' );
	}
}
