...
class WPF_GForms_Integration extends GFFeedAddOn {
	...
	public function init() {
		...
		add_action( 'gform_entry_list_action_wpf_process_again', array( $this, 'handle_bulk_process_again' ), 10, 3 );
	...
	public function process_feed( $feed, $entry, $form ) {
		...
		if ( ! empty( $field_mapping ) ) {
			...
			foreach ( $field_mapping as $crm_field => $source_field_or_tag ) {
				...
				$formatted_value = apply_filters( 'wpf_format_field_value', $value, $type, $crm_field, $update_data );

				if ( ! empty( $formatted_value ) || 0 === $formatted_value || '0' === $formatted_value || null === $formatted_value ) {
	 ...
	 * @return array|null The registration data or null to block it.
	 */
	public function maybe_bypass_user_register( $post_data ) {

		if ( isset( $_REQUEST['action'] ) && 'wp_gf_feed_processor' === $_REQUEST['action'] && ! did_action( 'gform_user_registered' ) ) {
			return null;
		}
	...
	public function maybe_process_entry() {
		...
		add_filter( 'gform_allow_feed_reprocessing', '__return_true' ); // allow feed reprocessing.
	...
	/**
	 * Add WP Fusion bulk action to entries list.
	 *
	 * @since 3.47.4.2
	 *
	...
	public function add_bulk_actions( $actions, $form_id ) {
		...
		// Only show if form has WP Fusion feeds.
		$feeds = GFAPI::get_feeds( null, $form_id, 'wpfgforms' );

		if ( ! empty( $feeds ) && ! is_wp_error( $feeds ) ) {
			$actions['wpf_process_again'] = __( 'Process WP Fusion actions again', 'wp-fusion' );
		}
	...
	/**
	 * Handle the bulk "Process WP Fusion actions again" action.
	 *
	 * Routes selected entries to the WP Fusion batch processor.
	 *
	 * @since 3.47.4.2
	 *
	 ...
	 * @param int    $form_id   The form ID.
	 */
	public function handle_bulk_process_again( $action, $entry_ids, $form_id ) {

		if ( empty( $entry_ids ) ) {
			return;
		}

		// Use WP Fusion's batch processor (matches WooCommerce pattern).
		wp_fusion()->batch->batch_init( 'gravity_forms', array( 'object_ids' => $entry_ids ) );
	}
