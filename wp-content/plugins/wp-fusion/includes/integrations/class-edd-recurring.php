...
class WPF_EDD_Recurring extends WPF_Integrations_Base {
	 ...
	 * @since 3.46.1  Accepts either a subscription ID or object.
	 * @since 3.46.11   Added optional status parameter to prevent timing issues.
	 *
	 * @param int|EDD_Subscription $subscription The subscription ID or object.
	 * @param string               $status       Optional. The subscription status to use instead of calling get_status().
	 */
	public function sync_subscription_fields( $subscription, $status = null ) {

		if ( is_numeric( $subscription ) ) {
			// Get the subscription object - use the namespaced class if available, otherwise fallback to legacy class.
			if ( class_exists( '\EDD\Recurring\Subscriptions\Subscription' ) ) {
				$subscription = new \EDD\Recurring\Subscriptions\Subscription( $subscription );
			} elseif ( class_exists( 'EDD_Subscription' ) ) {
				$subscription = new EDD_Subscription( $subscription );
			} else {
				return false;
			}
		}
		...
		$update_data = array(
			...
			'edd_sub_status'         => $status ? $status : $subscription->get_status(),
		...
		$download_data = array(
			...
			'edd_sub_status_' . $download_id         => $status ? $status : $subscription->get_status(),
	 ...
	 */
	public function get_subscription_end_date( $subscription_id ) {
	...
	public function subscription_status_change( $old_status, $status, $subscription ) {
		...
		if ( doing_action( 'edd_subscription_status_change' ) ) {
			...
			$this->sync_subscription_fields( $subscription, $status );
		}
		...
		if ( 'active' !== $status && 'cancelled' !== $status && $settings['remove_tags'] ) {
		...
		if ( ! empty( $subscription->price_id ) ) {
			...
			if ( $settings['remove_tags'] && 'active' !== $status && 'cancelled' !== $status ) {
	...
	public function maybe_sync_renewal_data( $subscription_id, $expiry, $subscription, $payment_id ) {
		...
		if ( wpf_is_field_active( array( 'edd_sub_end_date', 'edd_sub_renewal_date', 'order_date', 'edd_sub_renewal_date_' . $subscription->product_id, 'edd_sub_status_' . $subscription->product_id ) ) ) {
			$this->sync_subscription_fields( $subscription, 'active' );
		}
	...
	public function meta_box_content( $post, $settings ) {
		...
		echo '<h3>' . esc_html__( 'WP Fusion - Recurring Payments Settings', 'wp-fusion' ) . '</h3>';
	...
	/**
	 * Outputs WPF fields to variable price rows for recurring subscriptions
	 *
	 * @access public
	 * @param int   $key      The price ID.
	...
	public function download_table_price_row( $key, $args, $post_id ) {
		...
		// Define the default structure for price-specific settings.
		...
		$settings = wp_parse_args( get_post_meta( $post_id, 'wpf-settings-edd', true ), $price_settings );

		// Ensure each price ID has its own array
		foreach ( $price_settings as $setting_key => $default ) {
			if ( ! isset( $settings[ $setting_key ][ $key ] ) ) {
				$settings[ $setting_key ][ $key ] = array();
			}
		}

		$variable_price = edd_get_variable_prices( $post_id );
		$recurring      = isset( $variable_price[ $key ] ) && ! empty( $variable_price[ $key ]['recurring'] ) && 'yes' === $variable_price[ $key ]['recurring'];

		echo '<div class="wpf-edd-custom-price-option-section wpf-edd-recurring-options' . ( $recurring ? '' : ' hidden' ) . '">';

		echo '<span class="wpf-edd-custom-price-option-section-title">' . esc_html__( 'WP Fusion - Recurring Payments Settings', 'wp-fusion' ) . '</span>';

		echo '<div class="wpf-edd-settings-grid">';

		// Trialling.
		echo '<div class="wpf-edd-field">';
		echo '<label>' . esc_html__( 'Subscription In Trial', 'wp-fusion' ) . ':</label>';
		wpf_render_tag_multiselect(
			array(
		...
		// Completed.
		echo '<div class="wpf-edd-field">';
		echo '<label>' . esc_html__( 'Subscription Completed', 'wp-fusion' ) . ':</label>';
		wpf_render_tag_multiselect(
			array(
		...
		// Failing.
		echo '<div class="wpf-edd-field">';
		echo '<label>' . esc_html__( 'Subscription Failing', 'wp-fusion' ) . ':</label>';
		wpf_render_tag_multiselect(
			array(
		...
		// Expired.
		echo '<div class="wpf-edd-field">';
		echo '<label>' . esc_html__( 'Subscription Expired', 'wp-fusion' ) . ':</label>';
		wpf_render_tag_multiselect(
			array(
		...
		// Cancelled.
		echo '<div class="wpf-edd-field">';
		echo '<label>' . esc_html__( 'Subscription Cancelled', 'wp-fusion' ) . ':</label>';
		wpf_render_tag_multiselect(
			array(
