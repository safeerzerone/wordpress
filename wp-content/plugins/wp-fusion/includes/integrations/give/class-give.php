...
class WPF_Give extends WPF_Integrations_Base {
	...
	public function init() {
		...
		// Optin field for form builder v3.
		add_action( 'givewp_donation_form_schema', array( $this, 'add_optin_field_v3' ) );
	...
	private function get_form_fields( $form ) {
		...
		// If the legacy fields are set, return them.
		if ( ! empty( $fields ) ) {
			return $fields;
		}

		$fields = give()->form_meta->get_meta( $form->ID, 'formBuilderFields', true );

		if ( empty( $fields ) ) {
			return array();
		}

		$fields = parse_blocks( $fields );
		$fields = json_decode( $fields[0]['innerContent'][0] );
		$fields = $this->get_custom_fields( $fields );

		$returnable = array();
		foreach ( $fields as $field ) {
			$type = explode( '/', $field->name );
	 ...
	 * @return array
	 */
	private function get_custom_fields( $blocks ) {
		$returnable = array();
		foreach ( $blocks as $block ) {
			// phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			$inner_blocks    = isset( $block->innerBlocks ) ? $block->innerBlocks : array();
			$is_custom_field = str_contains( $block->name, 'givewp-form-field-manager' ) || 'givewp/text' === $block->name;

			if ( ! $is_custom_field && empty( $inner_blocks ) ) {
				continue;
			}

			if ( ! empty( $inner_blocks ) ) {
				$returnable = array_merge( $returnable, $this->get_custom_fields( $inner_blocks ) );
			}
	 ...
	 * @return mixed HTML Output.
	 */
	public function add_optin_field( $form_id ) {
	...
	public function register_admin_scripts() {
		...
		// Ensure $settings is an array to prevent PHP warnings
	...
	public function save_form_builder( $form ) {
		// Get the entire form settings JSON
		$settings_json = give_clean( $_POST['settings'] ?? '' );

		if ( empty( $settings_json ) ) {
			return;
		}

		$post_settings = json_decode( $settings_json, true );
		if ( ! $post_settings ) {
			return;
		}
		...
		// Extract WP Fusion settings from the form settings JSON
		foreach ( $this->default_settings as $key => $default_value ) {
			if ( isset( $post_settings[ $key ] ) ) {
				$wpf_settings[ $key ] = wpf_clean_tags( $post_settings[ $key ] );
			}
		}

		// Handle the 'enabled' setting if present
		if ( isset( $post_settings['wpf_enabled'] ) ) {
			$wpf_settings['enabled'] = $post_settings['wpf_enabled'];
		}

		// Handle donation level tags from blocks (your existing logic)
		$blocks_json = give_clean( $_POST['blocks'] ?? '' );
		if ( ! empty( $blocks_json ) ) {
			$blocks       = json_decode( $blocks_json, true );
			$amount_block = $this->get_donation_amount_block_data( $blocks );

			if ( ! empty( $amount_block['attributes']['tagLevels'] ) ) {
				...
				foreach ( $tag_levels as $index => $tag_level ) {
					...
					foreach ( $tag_level['tags'] as $tag ) {
						$wpf_settings['apply_tags_level'][ $index ][] = (string) $tag['value'];
					}
				}
			}
		}

		// Save the WP Fusion settings
		if ( ! empty( $wpf_settings ) ) {
			give_update_meta( $form->id, 'wpf_settings_give', $wpf_settings );
		}
	}
	...
	/**
	 * Add the optin field to the form builder v3.
	 *
	 * @since 3.46.10
	 *
	 * @param Give\Framework\FieldsAPI\Form $form The form object.
	 *
	 * @return void
	 */
	public function add_optin_field_v3( $form ) {
		if ( ! wpf_get_option( 'give_email_optin' ) ) {
			return;
		}

		$default = 'unchecked' !== wpf_get_option( 'give_email_optin_default' );
		$message = wpf_get_option( 'give_email_optin_message', __( 'I consent to receive marketing emails', 'wp-fusion' ) );

		$form_nodes   = $form->all();
		$last_section = $form->count() ? $form_nodes[ $form->count() - 1 ] : null;

		if ( $last_section && is_null( $form->getNodeByName( 'give_email_optin' ) ) ) {
			$field = Checkbox::make( 'give_email_optin' )
				->label( $message )
				->showInAdmin()
				->showInReceipt()
				->rules( 'boolean' )
