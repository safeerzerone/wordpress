...
class WPF_FluentBooking extends IntegrationManagerController {
	...
	public function pushIntegration( $integrations, $calendar_event_id ) {
		...
		$integrations[ $this->integrationKey ] = array(
			...
			'configure_title'       => __( 'Configuration not required!', 'wp-fusion' ),
			'global_configure_url'  => '#',
			'configure_message'     => __( 'It\s configured!', 'wp-fusion' ),
			'configure_button_text' => __( 'Set WP Fusion API', 'wp-fusion' ),
		);
	...
	public function getSettingsFields( $settings, $calendar_event_id ) {
		...
		$fields[] = array(
			...
			'field_label_local'  => __( 'Booking Field', 'wp-fusion' ),
			'primary_fields'     => array(
				array(
					'key'           => 'email',
					'label'         => __( 'Email Address', 'wp-fusion' ),
					'required'      => true,
		...
		$fields[] = array(
			'key'                => 'other_fields',
			'require_list'       => false,
			'label'              => __( 'Other Fields', 'wp-fusion' ),
			'tips'               => sprintf( __( 'Select which FluentBooking fields pair with their respective fields in %s.', 'wp-fusion' ), wp_fusion()->crm->name ),
			'component'          => 'dropdown_many_fields',
			'field_label_remote' => wp_fusion()->crm->name . ' Field',
			'field_label_local'  => __( 'FluentBooking Field', 'wp-fusion' ),
		...
		$fields[] = array(
			...
			'key'            => 'event_trigger',
			'options'        => array(
				'after_booking_scheduled'    => __( 'Booking Confirmed', 'wp-fusion' ),
				'booking_schedule_completed' => __( 'Booking Completed', 'wp-fusion' ),
				'after_booking_rescheduled'  => __( 'Booking Rescheduled', 'wp-fusion' ),
				'booking_schedule_cancelled' => __( 'Booking Canceled', 'wp-fusion' ),
			),
			'tips'           => __( 'Select in which booking stage you want to trigger this feed.', 'wp-fusion' ),
			'label'          => __( 'Event Trigger', 'wp-fusion' ),
			'component'      => 'checkbox-multiple-text',
			'checkbox_label' => __( 'Event Trigger For This Feed', 'wp-fusion' ),
		);
		...
		$fields[] = array(
			...
			'checkbox_label' => __( 'Enable This feed', 'wp-fusion' ),
		);
	...
	public function notify( $feed, $booking, $calendar_event ) {
		...
		$primary_values = array_filter( Arr::only( $data, array( 'first_name', 'last_name', 'email' ) ) );
