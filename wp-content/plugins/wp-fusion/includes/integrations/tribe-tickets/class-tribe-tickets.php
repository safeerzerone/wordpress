...
class WPF_Tribe_Tickets extends WPF_Integrations_Base {
	...
	public function add_meta_fields( $meta_fields ) {
		...
		$additional_fields = apply_filters( 'tribe_ext_tickets_additional_fields', array() );
	...
	public function get_attendee_meta( $attendee_id ) {
		...
		// Additional fields from Event Tickets Additional Fields extension.
		$additional_fields = apply_filters( 'tribe_ext_tickets_additional_fields', array() );

		if ( ! empty( $additional_fields ) ) {
			foreach ( $additional_fields as $field_id => $field_data ) {
				$prefixed_field_id = '_tribe_tickets_meta_' . $field_id;
				$field_value       = get_post_meta( $ticket_id, $prefixed_field_id, true );
				
				if ( ! empty( $field_value ) ) {
					$update_data[ $field_id ] = $field_value;
				}
			}
		}
	...
	public function render_attendee_column( $value, $item, $column ) {
		...
		if ( 'wp_fusion' === $column ) {
			...
			$contact_id  = get_post_meta( $attendee_id, WPF_CONTACT_ID_META_KEY, true );
			$complete    = get_post_meta( $attendee_id, '_wpf_attendee_complete', true );
