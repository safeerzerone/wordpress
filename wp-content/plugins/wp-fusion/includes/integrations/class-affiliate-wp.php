...
class WPF_AffiliateWP extends WPF_Integrations_Base {
	...
	public function wpf_column_value( $group ) {
		...
		?>
			...
			<td
				...
				if ( $linked_tag ) {
					?>
					<span class="wpf-tip wpf-tip-bottom" data-tip="<?php echo esc_attr( sprintf( __( 'Linked with %1$s tag %2$s', 'wp-fusion' ), wp_fusion()->crm->name, wpf_get_tag_label( $linked_tag[0] ) ) ); ?>">
						<?php echo wpf_logo_svg( 14 ); ?>
					</span>
					<?php
				} else {
					?>
					<span class="wpf-tip wpf-tip-bottom" data-tip="<?php esc_attr_e( 'Not linked with tag', 'wp-fusion' ); ?>">
						<span class="dashicons dashicons-minus" style="font-size: 14px; line-height: 1; color: #ccc;"></span>
					</span>
					<?php
				}
				?>
	...
	public function tags_modified( $user_id, $user_tags ) {
		...
		// First, determine which group the affiliate should be in based on their tags.
		...
		foreach ( $groups as $group_id ) {
			...
			if ( array_intersect( $settings, $user_tags ) ) {
				...
				} elseif ( 'active' !== affwp_get_affiliate_status( $affiliate_id ) && 'inactive' !== affwp_get_affiliate_status( $affiliate_id ) ) {
					affwp_set_affiliate_status( $affiliate_id, 'active' );
				}

				$target_group_id = $group_id;
				break; // Affiliates can only be in one group.
			}
		}
		...
		$current_group_id = affwp_get_affiliate_group_id( $affiliate_id );

		// If the affiliate should be in a different group, handle the transition.
		if ( $target_group_id && $current_group_id !== $target_group_id ) {

			// First, remove from current group if they're in one.
			if ( $current_group_id ) {
				$this->remove_affiliate_from_group( $affiliate_id, $current_group_id, $user_id, $awpdb, $affwp );
			}

			// Then add to the new group.
			$this->add_affiliate_to_group( $affiliate_id, $target_group_id, $user_id, $awpdb, $affwp );

		} elseif ( ! $target_group_id && $current_group_id ) {

			// If the affiliate has no matching tags but is in a group, remove them.
			$this->remove_affiliate_from_group( $affiliate_id, $current_group_id, $user_id, $awpdb, $affwp );
	 ...
	 *
	 * Helper method to add an affiliate to a specific group.
	 *
	 * @since 3.46.3
	 *
	 ...
	 * @param int                         $user_id      The user ID.
	 * @param \AffiliateWP\Connections\DB $awpdb        The connections database object.
	 * @param \AffiliateWP\Groups\DB      $affwp        The groups database object.
	 */
	private function add_affiliate_to_group( $affiliate_id, $group_id, $user_id, $awpdb, $affwp ) {
	 ...
	 *
	 * Helper method to remove an affiliate from a specific group.
	 *
	 * @since 3.46.3
	 *
	 * @param int                         $affiliate_id The affiliate ID.
	 ...
	 * @param \AffiliateWP\Groups\DB      $affwp        The groups database object.
	 */
	private function remove_affiliate_from_group( $affiliate_id, $group_id, $user_id, $awpdb, $affwp ) {
		...
		if ( ! empty( $settings ) ) {
			...
			wpf_log(
				...
				$user_id,
				sprintf(
					/* translators: 1: Group edit URL, 2: Group title */
					__( 'User removed from AffiliateWP group <a href="%1$s" target="_blank">%2$s</a> so they can be added to their new group.', 'wp-fusion' ),
					admin_url( 'admin.php?page=affiliate-wp-affiliate-groups&action=edit&group_id=' . $group_id ),
					$affwp->get_group_title( $group_id )
				),
				array( 'source' => 'affiliate-wp' )
		...
		foreach ( $connection_ids as $connection_id ) {
			...
			if ( empty( $connection ) || is_wp_error( $connection ) ) {
				continue;
			}
			...
			if ( $affiliate_id == $connection['affiliate'] ) {
				...
				break; // Only need to remove one connection since affiliates can only be in one group.
	...
	public function add_affiliate( $affiliate_id ) {
		...
		if ( 'active' === $affiliate->status ) {
			...
			if ( ! empty( $linked_tag ) && ! doing_action( 'wpf_tags_modified' ) ) {
				// Don't need to reapply this if it just triggered the activation.
				$apply_tags = array_merge( $apply_tags, $linked_tag );
			}
