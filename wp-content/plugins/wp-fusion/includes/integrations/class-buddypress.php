...
class WPF_BuddyPress extends WPF_Integrations_Base {
	...
	public function updated_profile( $user_id, $field_ids = array(), $errors = false, $old_values = array(), $new_values = array() ) {
		...
		foreach ( $new_values as $field_id => $value ) {
			// Get the field object to check the field type.
			$field = xprofile_get_field( $field_id );

			if ( $field && 'datebox' === $field->type ) {
				// For datebox fields, get the properly formatted value directly from the database.
				$user_meta[ "field_{$field_id}" ] = xprofile_get_field_data( $field_id, $user_id );
			} else {
	...
	public function set_user_meta( $user_meta, $user_id ) {
		...
		foreach ( $user_meta as $key => $value ) {
			...
			if ( strpos( $key, 'bbp_field_' ) !== false ) {
				...
				// Set the field visibility to the field's default visibility level.
				if ( function_exists( 'xprofile_get_field' ) && function_exists( 'xprofile_set_field_visibility_level' ) ) {
					$field_object = xprofile_get_field( $field_id );

					if ( $field_object && isset( $field_object->default_visibility ) ) {
						xprofile_set_field_visibility_level( $field_id, $user_id, $field_object->default_visibility );
					}
				}
	...
	public function removing_group_access( $user_id, $group_id ) {
		...
		if ( true == $bp_ld_sync__syncing_to_learndash ) {
			...
			remove_action( 'ld_removed_group_access', array( wp_fusion()->integrations->learndash->groups, 'removed_group_access' ), 10, 2 );
