	...
	public function update_entry( $entry, $order, $next_status ) {
		...
		foreach ( $feeds as $feed ) {
			if ( $integration->is_feed_condition_met( $feed, $form, $entry ) ) {
				...
				// Support old field mapping format.
				...
				// Support new field mapping format.
				if ( ! empty( $feed['meta']['wpf_field_mapping'] ) ) {
					$feed['meta']['wpf_field_mapping']['wpf_order_email'] = array(
						'key'   => wpf_get_lookup_field(),
						'value' => 'wpf_order_email',
					);
				}
