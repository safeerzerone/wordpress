...
class WPF_wpForo extends WPF_Integrations_Base {
	...
	public function filter_valid_subscribers( array $subscribers, array $topic, array $forum ): array {
		...
		// Load the wpForo access settings to evaluate subscriber permissions.
		$settings = get_option( 'wpf_wpforo_settings', array() );

		$forum_id = 0;

		if ( isset( $forum['forumid'] ) ) {
			$forum_id = absint( $forum['forumid'] );
		} elseif ( isset( $topic['forumid'] ) ) {
			$forum_id = absint( $topic['forumid'] );
		}

		// If we can't determine the forum, we can't verify access.
		if ( empty( $forum_id ) ) {
			return $subscribers;
		}
		...
		foreach ( $subscribers as $key => $subscriber ) {
			...
			$user_id    = isset( $subscriber['userid'] ) ? absint( $subscriber['userid'] ) : 0;
			$user_email = isset( $subscriber['user_email'] ) ? $subscriber['user_email'] : '';

			if ( ! $this->user_has_forum_access( $forum_id, $user_id, $user_email, $settings, false ) ) {
	...
	public function permissions_forum_can( $can, $do, $forumid, $groupid, $user_id = false ) {
		...
		if ( empty( $settings[ $forumid ]['hide'] ) ) {
			return $can;
		}
		...
		if ( ! $this->user_has_forum_access( absint( $forumid ), $user_id, '', $settings ) ) {
			$can = false;
		}
	...
	/**
	 * Determines whether a user can access a forum based on WP Fusion restrictions.
	 *
	 * @since 3.47.3
	 *
	 ...
	 * @param array       $settings                Optional. The forum access settings.
	 * @param bool        $fallback_to_current_user Optional. Whether to use the current user.
	 *                                              Defaults to true.
	 *
	 * @return bool Whether the user can access the forum.
	 */
	protected function user_has_forum_access( $forum_id, $user_id = 0, $user_email = '', $settings = array(), $fallback_to_current_user = true ) {
		...
		if ( empty( $settings ) || empty( $settings[ $forum_id ] ) ) {
			return true;
		}

		$forum_settings = wp_parse_args(
			$settings[ $forum_id ],
			array(
				'required_tags' => array(),
				'hide'          => false,
		...
		$base_required_tags = array();

		$base_page_id = (int) get_option( 'wpforo_pageid' );

		if ( $base_page_id ) {
			$base_page_settings = wp_fusion()->access->get_post_access_meta( $base_page_id );

			if ( ! empty( $base_page_settings['required_tags'] ) ) {
				$base_required_tags = (array) $base_page_settings['required_tags'];
			}
		}

		// Base page restrictions take precedence over forum-level restrictions.
		$forum_settings['required_tags'] = array_values( array_unique( array_filter( array_merge( $base_required_tags, (array) $forum_settings['required_tags'] ) ) ) );

		if ( empty( $forum_settings['required_tags'] ) ) {
			return true;
		}

		if ( $fallback_to_current_user && empty( $user_id ) ) {
			$user_id = wpf_get_current_user_id();
		}

		if ( empty( $user_id ) && ! empty( $user_email ) ) {
			$user_id = (int) wpf_get_user_id_by_email( $user_email );
		}
		...
		$has_access = wpf_has_tag( $forum_settings['required_tags'], $user_id );

		/**
		 * Filters whether a user can access a wpForo forum based on WP Fusion restrictions.
		 *
		 * @since 3.47.3
		 *
		 ...
		 * @param int    $forum_id       The forum ID.
		 * @param int    $user_id        The user ID.
		 * @param array  $forum_settings The forum settings.
		 * @param string $user_email     The user email.
		 */
		return (bool) apply_filters( 'wpf_wpforo_user_has_forum_access', $has_access, $forum_id, $user_id, $forum_settings, $user_email );
	}
