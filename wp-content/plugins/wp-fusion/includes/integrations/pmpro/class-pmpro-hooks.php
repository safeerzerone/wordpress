...
class WPF_PMPro_Hooks {
	...
	/**
	 * Gets the cancel reason from the current request or user meta.
	 *
	 * The PMPro Reason for Cancelling addon stores the reason in
	 * $_REQUEST['pmpro_cancel_reason'] during frontend cancellation,
	 * and saves it to 'pmpror4c_reason' user meta.
	 *
	 * @since 3.47.8
	 *
	 * @param int $user_id The user ID.
	 * @return string|null The cancel reason, or null if not available.
	 */
	private function get_cancel_reason( $user_id ) {

		// Primary: read from current request (frontend cancel form).
		if ( array_key_exists( 'pmpro_cancel_reason', $_REQUEST ) ) {
			$cancel_reason = sanitize_text_field( wp_unslash( (string) $_REQUEST['pmpro_cancel_reason'] ) );
			return '' !== $cancel_reason ? $cancel_reason : null;
		}

		// Fallback: read most recent entry from addon's user meta.
		// The addon stores each cancellation as a separate meta row, so we retrieve all.
		$reasons = get_user_meta( $user_id, 'pmpror4c_reason', false );

		if ( ! empty( $reasons ) ) {
			$latest = end( $reasons );

			if ( ! empty( $latest['reason'] ) ) {
				return sanitize_text_field( $latest['reason'] );
			}
		}
	...
	public function sync_pending_cancellation_level_fields( $user_id, $level ) {
		...
		// Include cancel reason if available (from Reason for Cancelling addon).
		$cancel_reason = $this->get_cancel_reason( $user_id );

		if ( ! empty( $cancel_reason ) ) {
			$update_data['pmpro_cancel_reason']                = $cancel_reason;
			$update_data[ 'pmpro_cancel_reason_' . $level_id ] = $cancel_reason;
		}
	...
	public function sync_cancelled_level_fields( $user_id, $level, $status ) {
		...
		// Include cancel reason if available (from Reason for Cancelling addon).
		// Only sync for user-initiated cancellations, not admin_cancelled/expired/inactive.
		if ( 'cancelled' === $status ) {
			$cancel_reason = $this->get_cancel_reason( $user_id );
