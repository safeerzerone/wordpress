			...
			foreach ( $fields as $field ) {
				...
				if ( isset( $connection['fields'][ $field['id'] ] ) && ! empty( $connection['fields'][ $field['id'] ]['crm_field'] ) ) {
					...
					if ( 'checkbox' === $field['type'] ) {
						...
						// For date fields, use the unix timestamp if available, otherwise use the date value.
						if ( ! empty( $field['unix'] ) ) {
							$field['value'] = $field['unix'];
						} elseif ( ! empty( $field['date'] ) ) {
							$date_string = $field['date'];
							if ( ! empty( $field['time'] ) ) {
								$date_string .= ' ' . $field['time'];
							}

							$timestamp = strtotime( trim( $date_string ) );
					...
					$update_data[ $crm_field ] = apply_filters( 'wpf_format_field_value', $field['value'], $field['type'], $crm_field, $update_data );
