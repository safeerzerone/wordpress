...
class WPF_Woocommerce extends WPF_Integrations_Base {
	...
	public function init() {
		...
		add_filter( 'woocommerce_order_item_name', array( $this, 'rename_order_item_coupon' ), 10, 2 );
		add_filter( 'woocommerce_add_message', array( $this, 'filter_coupon_notices' ) );
		add_filter( 'woocommerce_add_success', array( $this, 'filter_coupon_notices' ) );
	...
	public function get_contact_id_from_order( $order ) {
		...
		if ( $user_id ) {
			...
			if ( is_object( $userdata ) && $userdata->user_email === $order->get_billing_email() ) {
	...
	public function get_customer_data( $order ) {
		...
		// Lead source tracking variables should only come from cookies, not order meta.
		// External systems like Zapier might incorrectly save these to order meta.
		...
		foreach ( $order_data['meta_data'] as $meta ) {
			...
			if ( is_a( $meta, 'WC_Meta_Data' ) ) {
				...
				// Skip lead source tracking fields from order meta.
				$clean_key = ltrim( $data['key'], '_' );

				if ( in_array( $clean_key, $skip_meta_keys, true ) ) {
					continue;
				}
		...
		foreach ( $customer_data as $key => $value ) {
			...
			if ( 'billing_email' == $key ) {
				...
				} elseif ( wpf_get_crm_field( 'billing_first_name' ) === wpf_get_crm_field( 'first_name' ) || empty( $user_id ) ) {
				...
				} elseif ( wpf_get_crm_field( 'billing_last_name' ) === wpf_get_crm_field( 'last_name' ) || empty( $user_id ) ) {
	 ...
	 * @return string|false The contact ID created / updated, or false on failure.
	 */
	...
	public function handle_bulk_actions( $ids, $action, $post_type ) {
		...
		if ( false !== strpos( $action, 'mark_' ) ) {
			...
			// Don't run the normal order status change actions in bulk as they could time out.

			remove_action( 'woocommerce_order_status_changed', array( $this, 'order_status_changed' ), 2 );
	...
	public function order_status_changed( $order_id, $old_status, $new_status, $order ) {
		...
		// If the order hasn't been processed at all, this can be handled by the main process_order() function.
		if ( ! $order->get_meta( 'wpf_complete', true ) && in_array( $new_status, $this->get_valid_order_statuses(), true ) ) {
			$this->process_order( $order_id );
			return;
		}
		...
		$order->add_order_note( 'WP Fusion order status changed actions completed for status ' . $new_status . '.' );
	}
	...
	/**
	 * Renames coupon codes in order displays (emails, order details, admin).
	 *
	 * @since 3.47.5
	 *
	 * @param string        $name The order item name.
	 * @param WC_Order_Item $item The order item object.
	 * @return string The modified name.
	 */
	public function rename_order_item_coupon( $name, $item ) {

		// Only modify coupon items.
		if ( 'coupon' !== $item->get_type() ) {
			return $name;
		}

		// Get the coupon code from the order item.
		$coupon_code = $item->get_code();

		// Look up the original coupon by code.
		$coupon_id = wc_get_coupon_id_by_code( $coupon_code );

		if ( ! $coupon_id ) {
			return $name; // Coupon no longer exists, keep original.
		}
	...
	/**
	 * Filters WooCommerce notice messages to replace coupon codes with custom labels.
	 *
	 * This catches coupon-related notices that aren't filtered by woocommerce_coupon_message,
	 * such as the empty cart notice: "Coupon code "[code]" applied successfully..."
	 *
	 * @since 3.47.6
	 *
	 ...
	 */
	public function filter_coupon_notices( $message ) {
		...
		foreach ( $applied_coupons as $coupon_code ) {
			...
			// Replace coupon code with custom label if configured.
			if ( ! empty( $settings['coupon_label'] ) ) {

				// Handle quoted coupon codes (e.g. "COUPONCODE").
				$message = str_replace(
					'"' . $coupon_code . '"',
					'"' . $settings['coupon_label'] . '"',
					$message
				);

				// Handle unquoted coupon codes.
				$message = str_replace(
					$coupon_code,
					$settings['coupon_label'],
					$message
			...
			// Replace entire message with custom message if configured.
			if ( ! empty( $settings['coupon_applied_msg'] ) ) {

				// Check if this notice is about a coupon being applied.
				// We use 'woocommerce' textdomain intentionally to match their default message.
				$default_pattern = __( 'Coupon code applied successfully.', 'woocommerce' ); // phpcs:ignore WordPress.WP.I18n.TextDomainMismatch

				if ( strpos( $message, $default_pattern ) !== false || strpos( $message, $coupon_code ) !== false ) {
					return $settings['coupon_applied_msg'];
				}
			}
		}
