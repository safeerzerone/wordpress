...
class WPF_Forms_Helper {
	...
	public static function process_form_data( $args ) {
		...
		$update_data = apply_filters( 'wpf_forms_pre_submission', $update_data, $user_id, $contact_id, $form_id, $entry_id );
		$update_data = apply_filters( 'wpf_' . $integration_slug . '_pre_submission', $update_data, $user_id, $contact_id, $form_id, $entry_id );
		...
		if ( ! empty( $update_data ) ) {
			...
			if ( ! empty( $contact_id ) ) {
				...
				if ( is_wp_error( $contact_id ) ) {
					...
					// Disable API queue for new guest contact creation. Can cause issues with user registration.
