...
class WPF_Pipedrive_Admin {
	...
	private function show_revoke_confirmation_page( $connection_id, $return_url ) {
		...
		<head>
			...
			<title><?php esc_html_e( 'Revoke Connection', 'wp-fusion' ); ?></title>
			<style>
				body {
		...
		<body>
			<div class="container">
				<h1><?php esc_html_e( 'Revoke Pipedrive Connection', 'wp-fusion' ); ?></h1>
				<p><?php esc_html_e( 'Are you sure you want to revoke the connection to Pipedrive? This will:', 'wp-fusion' ); ?></p>
				<ul>
					<li><?php esc_html_e( 'Remove all stored access tokens', 'wp-fusion' ); ?></li>
					<li><?php esc_html_e( 'Disable the Pipedrive integration', 'wp-fusion' ); ?></li>
					<li><?php esc_html_e( 'Require re-authorization to reconnect', 'wp-fusion' ); ?></li>
				</ul>
				...
				<form method="post" id="revoke-form">
					...
					<div class="buttons">
						<button type="submit" class="button button-primary" id="confirm-button">
							<?php esc_html_e( 'Yes, Revoke Connection', 'wp-fusion' ); ?>
						</button>
						<button type="button" class="button button-secondary" onclick="window.close();">
							<?php esc_html_e( 'Cancel', 'wp-fusion' ); ?>
						</button>
					</div>
				</form>
				
				<div class="loading" id="loading">
					<p><?php esc_html_e( 'Revoking connection...', 'wp-fusion' ); ?></p>
				</div>
			</div>
	...
	private function send_revoke_response( $success, $message, $return_url ) {
		...
		<head>
			<meta charset="<?php bloginfo( 'charset' ); ?>">
			<meta name="viewport" content="width=device-width, initial-scale=1">
			<title><?php esc_html_e( 'Connection Status', 'wp-fusion' ); ?></title>
			<style>
				body {
					font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen-Sans, Ubuntu, Cantarell, "Helvetica Neue", sans-serif;
					margin: 0;
		...
		<body>
			<div class="container">
				<h1 class="<?php echo $success ? 'success' : 'error'; ?>">
					<?php echo $success ? esc_html__( 'Success', 'wp-fusion' ) : esc_html__( 'Error', 'wp-fusion' ); ?>
				</h1>
				<p><?php echo esc_html( $message ); ?></p>
				<button type="button" class="button" onclick="window.close();">
					<?php esc_html_e( 'Close Window', 'wp-fusion' ); ?>
				</button>
			</div>
	...
	public function register_settings( $settings, $options ) {
		...
		if ( wp_fusion()->is_full_version() ) {
			...
			$settings['access_key_desc'] = array(
				'type'    => 'paragraph',
				'section' => 'main',
				'desc'    => __( 'Configuring the fields below allows you to add new users to your site and update existing users based on changes in Pipedrive. Read our <a href="https://wpfusion.com/documentation/webhooks/pipedrive-webhooks/" target="_blank">documentation</a> for more information.', 'wp-fusion' ),
			);

			$settings['access_key_desc']['desc'] .= ' ' . sprintf( __( 'To list all registered webhooks (for debugging purposes), %1$sclick here%2$s.', 'wp-fusion' ), '<a href="' . esc_url( admin_url( 'options-general.php?page=wpf-settings&pd_debug=true' ) ) . '">', '</a>' );

			if ( isset( $_GET['pd_debug'] ) ) {

				$settings['access_key_desc']['desc'] .= ' ' . sprintf( __( 'To <strong>delete</strong> all registered webhooks, %1$sclick here%2$s.', 'wp-fusion' ), '<a href="' . esc_url( admin_url( 'options-general.php?page=wpf-settings&pd_debug=true&pd_destroy_all_webhooks=true' ) ) . '">', '</a>' );
			...
			$new_settings['pipedrive_update_trigger'] = array(
				...
				'desc'    => __( 'When a person is updated in Pipedrive, send their data back to WordPress.', 'wp-fusion' ),
				'type'    => 'checkbox',
				'section' => 'main',
			);
			...
			$new_settings['pipedrive_add_trigger'] = array(
				'title'   => __( 'Add Trigger', 'wp-fusion' ),
				'desc'    => __( 'When a new person is created in Pipedrive, import them as a new WordPress user.', 'wp-fusion' ),
				'type'    => 'checkbox',
				'section' => 'main',
			);
	...
	public function register_connection_settings( $settings, $options ) {
		...
		if ( empty( $options[ "{$this->slug}_refresh_token" ] ) && ! isset( $_GET['code'] ) ) {
			...
			$new_settings['pipedrive_tag'] = array(
				'title'   => __( 'Segmentation Field', 'wp-fusion' ),
				'type'    => 'crm_field',
				'section' => 'setup',
				'desc'    => __( 'Select a field to be used for segmentation with WP Fusion. For more information, see <a href="https://wpfusion.com/documentation/crm-specific-docs/pipedrive-labels/" target="_blank">Tags with Pipedrive</a>.', 'wp-fusion' ),
			);
