...
class WPF_NationBuilder {
	...
	public function api_key_warning() {
		...
		$message = sprintf(
			...
			__( '<strong>Heads up:</strong> NationBuilder now uses OAuth for the v2 API. Please re-authorize your connection on the <a href="%s">Setup tab</a> to avoid service interruption.', 'wp-fusion' ),
	...
	public function add_contact_to_list( $contact_id, $list_id ) {
		...
		if ( 0 === $contact_id || 0 === $list_id ) {
			return new WP_Error( 'invalid_parameter', __( 'Invalid list or contact ID.', 'wp-fusion' ) );
		}
	...
	protected function remove_tags_v2( $tags, $contact_id ) {
		...
		if ( ! empty( $path_ids ) ) {
			wpf_log(
				'notice',
				$contact_id,
				__( 'Path journeys cannot be removed via the NationBuilder API. Skipping:', 'wp-fusion' ),
	...
	protected function add_contact_v2( $data ) {
		...
		return new WP_Error( 'error', __( 'Unable to create contact.', 'wp-fusion' ) );
	}
