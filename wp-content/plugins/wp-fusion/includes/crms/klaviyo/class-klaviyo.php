...
class WPF_Klaviyo {
			...
			if ( ( 401 === $response_code || 403 === $response_code ) && false === strpos( $url, 'oauth/token' ) ) {
				...
				if ( isset( $body_json->errors[0]->detail ) && false !== strpos( $body_json->errors[0]->detail, 'authentication' ) ) {
					...
					if ( is_wp_error( $access_token ) ) {
						return new WP_Error( 'error', sprintf( __( 'Error refreshing access token: %s', 'wp-fusion' ), $access_token->get_error_message() ) );
					}
		...
		while ( $continue ) {
			...
			foreach ( $response->data as $list ) {
				...
				$available_tags[ $list->id . '_optin' ] = array(
					'label'    => $list->attributes->name . ' ' . __( '(opt-in to marketing)', 'wp-fusion' ),
					'category' => 'Lists',
				);
			}
	...
	public function add_contact( $data ) {
		...
		if ( strlen( $params['body'] ) > 102400 ) {
			return new WP_Error( 'payload_too_large', __( 'Profile payload exceeds 100KB limit', 'wp-fusion' ) );
		}
