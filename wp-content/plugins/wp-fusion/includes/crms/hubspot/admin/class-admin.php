...
class WPF_HubSpot_Admin {
	...
	public function register_connection_settings( $settings, $options ) {
		...
		if ( empty( $options['hubspot_token'] ) && ! isset( $_GET['code'] ) ) {
			...
			$new_settings['hubspot_oauth_status'] = array(
				'title'       => __( 'Connection Status', 'wp-fusion' ),
			...
			if ( ! empty( $options['connection_configured'] )
				...
				$new_settings['hubspot_migration_header'] = array(
					'title'   => __( 'HubSpot List ID Migration', 'wp-fusion' ),
					'type'    => 'heading',
					'section' => 'setup',
				);
	...
	public function migration_notice() {
		...
		<div id="wpf-hubspot-migration" class="notice notice-warning">
			<p>
				<strong><?php esc_html_e( 'WP Fusion', 'wp-fusion' ); ?>:</strong>
				<?php esc_html_e( 'WP Fusion is currently using HubSpot\'s legacy v1 Lists API for compatibility.', 'wp-fusion' ); ?>
			</p>
		<p>
			<?php esc_html_e( 'Run this migration to switch to the v3 Lists API and update saved list references before April 30, 2026.', 'wp-fusion' ); ?>
		</p>
		<p>
			<strong><?php esc_html_e( 'Please back up your database before proceeding.', 'wp-fusion' ); ?></strong>
			<?php esc_html_e( 'This migration will update HubSpot list ID references stored across your posts, users, taxonomy rules, and plugin settings.', 'wp-fusion' ); ?>
		</p>
		<p>
			<button id="wpf-run-migration" class="button button-primary">
				<?php esc_html_e( 'Run Migration', 'wp-fusion' ); ?>
			</button>
		...
		(function() {
			...
			if ( ! btn ) {
				...
				btn.addEventListener( 'click', function() {
					btn.disabled = true;
					status.textContent = '<?php echo esc_js( __( 'Starting migration...', 'wp-fusion' ) ); ?>';
				...
				fetch( ajaxurl, { method: 'POST', body: data, credentials: 'same-origin' } )
					...
					.then( function( r ) {
						if ( ! r.success ) {
							status.textContent = r.data && r.data.message
								? r.data.message
								: '<?php echo esc_js( __( 'Migration failed.', 'wp-fusion' ) ); ?>';
							btn.disabled = false;
							return;
							}
							...
							if ( r.data.requires_confirm ) {
								if ( ! window.confirm( r.data.summary || '<?php echo esc_js( __( 'Proceed with migration?', 'wp-fusion' ) ); ?>' ) ) {
									status.textContent = '<?php echo esc_js( __( 'Migration cancelled.', 'wp-fusion' ) ); ?>';
									btn.disabled = false;
									return;
								}
							...
							if ( r.data.done ) {
								...
								wrap.innerHTML = '<p><strong><?php echo esc_js( __( 'WP Fusion', 'wp-fusion' ) ); ?>:</strong> '
								+ '<?php echo esc_js( __( 'HubSpot list ID migration completed successfully.', 'wp-fusion' ) ); ?>'
								+ '</p>';
								return;
							}
					...
					.catch( function() {
						status.textContent = '<?php echo esc_js( __( 'An unexpected error occurred.', 'wp-fusion' ) ); ?>';
						btn.disabled = false;
					});
			}
		})();
	...
	public function ajax_migrate_list_ids() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'wp-fusion' ) ) );
		}
		...
		switch ( $step ) {
			...
			case 'reset':
				...
				wp_send_json_success(
					array(
						...
						'cursor'    => 0,
						'message'   => __( 'Starting migration...', 'wp-fusion' ),
					)
				);
			...
			case 'scan_posts':
				...
				if ( 0 === $result['next_cursor'] ) {
					wp_send_json_success(
						array(
							...
							'cursor'    => 0,
							'message'   => __( 'Post meta scanned. Scanning user meta...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						...
						'message'   => __( 'Scanning post meta...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'scan_users':
				...
				if ( 0 === $result['next_cursor'] ) {
					...
					wp_send_json_success(
						array(
							...
							'cursor'    => 0,
							'message'   => __( 'User meta scanned. Scanning term meta...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						'next_step' => 'scan_users',
						'cursor'    => $result['next_cursor'],
						'message'   => __( 'Scanning user meta...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'scan_termmeta':
				...
				if ( 0 === $result['next_cursor'] ) {
					wp_send_json_success(
						array(
							...
							'cursor'    => 0,
							'message'   => __( 'Term meta scanned. Scanning options...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						'next_step' => 'scan_termmeta',
						'cursor'    => $result['next_cursor'],
						'message'   => __( 'Scanning term meta...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'scan_options':
				...
				if ( 0 === $result['next_cursor'] ) {
					wp_send_json_success(
						array(
							...
							'cursor'    => 0,
							'message'   => __( 'Options scanned. Scanning WP Fusion settings...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						'next_step' => 'scan_options',
						'cursor'    => $result['next_cursor'],
						'message'   => __( 'Scanning options...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'scan_wpf_options':
				...
				wp_send_json_success(
					array(
						...
						'cursor'    => 0,
						'message'   => __( 'WP Fusion settings scanned. Scanning custom tables...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'scan_custom_rcp_groupmeta':
				...
				if ( 0 === $result['next_cursor'] ) {
					...
					$message   = 'scan_integrations' === $next_step
						? __( 'Custom tables scanned. Scanning integrations...', 'wp-fusion' )
						: __( 'Scanning custom tables...', 'wp-fusion' );
				...
				wp_send_json_success(
					array(
						'next_step' => $step,
						'cursor'    => $result['next_cursor'],
						'message'   => __( 'Scanning custom tables...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'scan_integrations':
				...
				if ( 0 === $result['next_cursor'] ) {
					wp_send_json_success(
						array(
							'next_step' => 'map',
							'cursor'    => 0,
							'message'   => __( 'Scan complete. Fetching ID mapping from HubSpot...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						...
						'cursor'    => $result['next_cursor'],
						'message'   => __( 'Scanning integrations...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'map':
				...
				if ( 0 === $result ) {
					...
					wp_send_json_success(
						array(
							'done'    => true,
							'message' => __( 'No legacy IDs found that need migration.', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						...
						'message'   => sprintf( __( '%d list IDs mapped. Preparing preflight summary...', 'wp-fusion' ), $result ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'preflight':
				...
				if ( empty( $id_map ) || ! is_array( $id_map ) ) {
					wp_send_json_error( array( 'message' => __( 'No migration map available.', 'wp-fusion' ) ) );
				}
				...
				$summary_template = __( 'Dry run complete. %1$d IDs can be migrated across approximately %2$d rows. Continue?', 'wp-fusion' );
				...
				wp_send_json_success(
					array(
						...
						'message'              => __( 'Dry run complete. Confirmation required.', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'confirm':
				if ( ! $confirmed ) {
					wp_send_json_error( array( 'message' => __( 'Migration confirmation required.', 'wp-fusion' ) ) );
				}
				...
				if ( empty( $confirm_token ) || empty( $stored_token ) || $confirm_token !== $stored_token ) {
					wp_send_json_error( array( 'message' => __( 'Invalid confirmation token.', 'wp-fusion' ) ) );
				}
				...
				wp_send_json_success(
					array(
						...
						'cursor'    => 0,
						'message'   => __( 'Confirmation received. Updating post meta...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'update_posts':
				if ( ! $this->is_migration_confirmed() ) {
					wp_send_json_error( array( 'message' => __( 'Migration must be confirmed before writing.', 'wp-fusion' ) ) );
				}
				...
				if ( 0 === $next_cursor ) {
					wp_send_json_success(
						array(
							...
							'cursor'    => 0,
							'message'   => __( 'Post meta updated. Updating user meta...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						'next_step' => 'update_posts',
						'cursor'    => $next_cursor,
						'message'   => __( 'Updating post meta...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'update_users':
				...
				if ( 0 === $next_cursor ) {
					wp_send_json_success(
						array(
							...
							'cursor'    => 0,
							'message'   => __( 'User meta updated. Updating term meta...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						'next_step' => 'update_users',
						'cursor'    => $next_cursor,
						'message'   => __( 'Updating user meta...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'update_termmeta':
				...
				if ( 0 === $result['next_cursor'] ) {
					wp_send_json_success(
						array(
							...
							'cursor'    => 0,
							'message'   => __( 'Term meta updated. Updating options...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						'next_step' => 'update_termmeta',
						'cursor'    => $result['next_cursor'],
						'message'   => __( 'Updating term meta...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'update_options':
				...
				if ( 0 === $result['next_cursor'] ) {
					wp_send_json_success(
						array(
							...
							'cursor'    => 0,
							'message'   => __( 'Options updated. Updating WP Fusion settings...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						'next_step' => 'update_options',
						'cursor'    => $result['next_cursor'],
						'message'   => __( 'Updating options...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'update_wpf_options':
				...
				wp_send_json_success(
					array(
						...
						'cursor'    => 0,
						'message'   => __( 'WP Fusion settings updated. Updating custom tables...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'update_custom_rcp_groupmeta':
				...
				if ( 0 === $result['next_cursor'] ) {
					...
					$message   = 'update_integrations' === $next_step
						? __( 'Custom tables updated. Updating integrations...', 'wp-fusion' )
						: __( 'Updating custom tables...', 'wp-fusion' );
				...
				wp_send_json_success(
					array(
						'next_step' => $step,
						'cursor'    => $result['next_cursor'],
						'message'   => __( 'Updating custom tables...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'update_integrations':
				...
				if ( 0 === $result['next_cursor'] ) {
					wp_send_json_success(
						array(
							'next_step' => 'finalize',
							'cursor'    => 0,
							'message'   => __( 'All settings updated. Finalizing...', 'wp-fusion' ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						...
						'cursor'    => $result['next_cursor'],
						'message'   => __( 'Updating integrations...', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable
			...
			case 'finalize':
				...
				wp_send_json_success(
					array(
						'done'    => true,
						'message' => __( 'Migration completed successfully.', 'wp-fusion' ),
					)
				);
				break; // @phpstan-ignore deadCode.unreachable

			default:
				wp_send_json_error( array( 'message' => __( 'Unknown migration step.', 'wp-fusion' ) ) );
		}
	}
	...
	private function fetch_legacy_id_mapping() {
		...
		if ( empty( $this->crm->params ) ) {
			return new WP_Error( 'auth', __( 'Unable to connect to HubSpot.', 'wp-fusion' ) );
		}
	...
	public function render_migration_settings_widget( $id = '', $field = array() ) {
		...
		if ( $migrated ) {
			...
			esc_html_e( 'Migration complete. Your site is using the HubSpot v3 Lists API.', 'wp-fusion' );
			echo '</p>';
			return;
		}
		...
		<div id="wpf-hubspot-migration-settings">
			<p>
				<?php esc_html_e( 'Your site has legacy HubSpot v1 list IDs stored in the database that need to be updated to v3 IDs.', 'wp-fusion' ); ?>
			</p>
			<p>
				<strong><?php esc_html_e( 'Please back up your database before proceeding.', 'wp-fusion' ); ?></strong>
				<?php esc_html_e( 'This migration will update list ID references across all posts, users, taxonomy rules, and plugin settings.', 'wp-fusion' ); ?>
			</p>
			<p>
						...
						if ( ! r.success ) {
							...
							if ( r.data.done ) {
								wrap.innerHTML = '<p class="description" style="color:#46b450;font-weight:bold;">'
									+ '<?php echo esc_js( __( 'Migration completed successfully.', 'wp-fusion' ) ); ?>'
								+ '</p>';
								return;
							}
