...
class WPF_HighLevel_Admin {
	...
	public function check_oauth_conflicts() {
		...
		if ( ! empty( $conflicts ) ) {
			...
			$message = '<strong>' . esc_html__( 'OAuth Conflict Warning:', 'wp-fusion' ) . '</strong> ';

			if ( count( $conflicts ) === 1 ) {
				$message .= sprintf(
					// translators: %1$s is the name of the conflicting plugin.
					__( 'The %1$s plugin may interfere with HighLevel authorization. If you experience connection issues, try temporarily disabling this plugin during setup.', 'wp-fusion' ),
					'<strong>' . esc_html( $conflicts[0] ) . '</strong>'
				);
			} else {
				$message .= sprintf(
					// translators: %1$s is a comma-separated list of conflicting plugin names.
					__( 'The following plugins may interfere with HighLevel authorization: %1$s. If you experience connection issues, try temporarily disabling these plugins during setup.', 'wp-fusion' ),
