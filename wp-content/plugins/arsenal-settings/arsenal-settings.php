...
require_once __DIR__ . '/arsenal-settings-block-concurrent-membership-purchase.php';
...
/**
 * Find an existing successful ARMember WooCommerce payment-log row for the same WC Stripe renewal (dedupe).
 *
 * ARMember may log `arm_transaction_id` as the WooCommerce order id, as a Stripe charge / PaymentIntent id, or only
 * store Stripe ids inside `arm_extra_vars`. Without this wider match, the backfill can insert a second success row and/or
 * the failed-log repair can convert a system "failed" row into a duplicate success alongside the backfill row.
 *
 * @param string $table         Payment log table name.
 * @param int    $user_id       WordPress user id.
 * @param int    $plan_id       ARMember plan id.
 * @param int    $order_id      WooCommerce order id (optional when Stripe charge/intent ids are supplied).
 * @param string $stripe_charge        Stripe charge id from the order (optional).
 * @param string $stripe_intent        Stripe PaymentIntent id from the order (optional).
 * @param string $stripe_subscription  Stripe subscription id (optional).
 * @param string $stripe_invoice       Stripe invoice id (optional).
 * @return int Existing `arm_log_id` or 0.
 */
function arsenal_settings_arm_find_existing_wc_stripe_success_payment_log_id( $table, $user_id, $plan_id, $order_id, $stripe_charge = '', $stripe_intent = '', $stripe_subscription = '', $stripe_invoice = '' ) {
	global $wpdb;
	...
	$table    = (string) $table;
	if ( $user_id < 1 || $plan_id < 1 || '' === $table ) {
		return 0;
	}

	$stripe_charge        = is_string( $stripe_charge ) ? $stripe_charge : '';
	$stripe_intent        = is_string( $stripe_intent ) ? $stripe_intent : '';
	$stripe_subscription  = is_string( $stripe_subscription ) ? $stripe_subscription : '';
	$stripe_invoice       = is_string( $stripe_invoice ) ? $stripe_invoice : '';

	$txn_values = array();
	if ( $order_id > 0 ) {
		$txn_values[] = (string) $order_id;
	}
	foreach ( array( $stripe_charge, $stripe_intent, $stripe_invoice, $stripe_subscription ) as $stripe_id ) {
		if ( '' !== $stripe_id ) {
			$txn_values[] = $stripe_id;
		}
	}
	$txn_values = array_values( array_unique( array_filter( $txn_values, function ( $v ) {
		return '' !== (string) $v;
	} ) ) );
	...
	if ( ! empty( $txn_values ) ) {
		$placeholders = implode( ',', array_fill( 0, count( $txn_values ), '%s' ) );
		$or_parts[]   = "`arm_transaction_id` IN ({$placeholders})";
		foreach ( $txn_values as $tv ) {
			$params[] = (string) $tv;
		}
	}
	...
	if ( $order_id > 0 ) {
		...
		$params[]   = '%' . $wpdb->esc_like( '"woocommerce_order_id";i:' . $order_id . ';' ) . '%';
	}

	$stripe_like_ids = array(
		$stripe_charge       => '/^ch_[a-zA-Z0-9]+$/',
		$stripe_intent       => '/^pi_[a-zA-Z0-9]+$/',
		$stripe_invoice      => '/^in_[a-zA-Z0-9]+$/',
		$stripe_subscription => '/^sub_[a-zA-Z0-9]+$/',
	);
	foreach ( $stripe_like_ids as $stripe_id => $pattern ) {
		if ( '' !== $stripe_id && preg_match( $pattern, $stripe_id ) ) {
			$or_parts[] = '`arm_extra_vars` LIKE %s';
			$params[]   = '%' . $wpdb->esc_like( $stripe_id ) . '%';
		}
	}

	if ( '' !== $stripe_subscription && preg_match( '/^sub_[a-zA-Z0-9]+$/', $stripe_subscription ) ) {
		$or_parts[] = '`arm_token` = %s';
		$params[]   = $stripe_subscription;
	}
	...
	// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name from ARMember helper; OR clause built from fixed patterns.
	$sql = "SELECT `arm_log_id` FROM `{$table}` WHERE `arm_user_id` = %d AND `arm_plan_id` = %d AND `arm_payment_gateway` = %s AND `arm_transaction_status` = %s AND ( {$where_or} ) ORDER BY `arm_log_id` ASC LIMIT 1";

	// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Dynamic OR list; values are passed via prepare.
...
/**
 * Build a stable grouping key for duplicate WooCommerce success payment-log rows.
 *
 * @param array $row Payment-log row.
 * @return string
 */
function arsenal_settings_arm_payment_log_dedupe_key( array $row ) {
	$user_id = isset( $row['arm_user_id'] ) ? (int) $row['arm_user_id'] : 0;
	$plan_id = isset( $row['arm_plan_id'] ) ? (int) $row['arm_plan_id'] : 0;
	$extra   = arsenal_settings_arm_payment_log_extra_vars_array( isset( $row['arm_extra_vars'] ) ? $row['arm_extra_vars'] : '' );

	$refs = array();
	$txn  = isset( $row['arm_transaction_id'] ) ? (string) $row['arm_transaction_id'] : '';
	if ( preg_match( '/^(ch_|pi_|in_|sub_)[a-zA-Z0-9]+$/', $txn ) ) {
		$refs[] = $txn;
	}
	foreach ( array( 'stripe_charge_id', 'stripe_payment_intent_id', 'stripe_invoice_id', 'stripe_subscription_id' ) as $meta_key ) {
		if ( ! empty( $extra[ $meta_key ] ) ) {
			$refs[] = (string) $extra[ $meta_key ];
		}
	}
	if ( ! empty( $extra['woocommerce_order_id'] ) ) {
		$refs[] = 'wc_order:' . (int) $extra['woocommerce_order_id'];
	}
	$token = isset( $row['arm_token'] ) ? (string) $row['arm_token'] : '';
	if ( preg_match( '/^sub_[a-zA-Z0-9]+$/', $token ) ) {
		$refs[] = $token;
	...
	if ( ! empty( $refs ) ) {
		return $user_id . '|' . $plan_id . '|' . implode( '|', $refs );
	}

	$created = isset( $row['arm_created_date'] ) ? substr( (string) $row['arm_created_date'], 0, 16 ) : '';
	$amount  = isset( $row['arm_amount'] ) ? number_format( (float) $row['arm_amount'], 2, '.', '' ) : '0.00';

	return $user_id . '|' . $plan_id . '|' . $created . '|' . $amount;
}

/**
 * Score a payment-log row when picking which duplicate to keep (higher is better).
 *
 * @param array $row Payment-log row.
 * @return int
 */
...
/**
 * Remove duplicate successful WooCommerce payment-log rows (same renewal / Stripe payment).
 *
 * @param int $days Look-back window in days.
 * @return int Number of rows deleted.
 */
function arsenal_settings_arm_dedupe_duplicate_wc_success_payment_logs( $days = 30 ) {
	global $wpdb;
	...
	$rows = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT * FROM `{$table}` WHERE `arm_payment_gateway` = %s AND `arm_transaction_status` = %s AND `arm_created_date` >= %s ORDER BY `arm_log_id` ASC",
			'woocommerce',
			'success',
	...
	$groups = array();
	foreach ( $rows as $row ) {
		$key = arsenal_settings_arm_payment_log_dedupe_key( $row );
		if ( ! isset( $groups[ $key ] ) ) {
			$groups[ $key ] = array();
		}
		$groups[ $key ][] = $row;
	...
	foreach ( $groups as $key => $group_rows ) {
		...
		usort(
			$group_rows,
			function ( $a, $b ) {
				$score_cmp = arsenal_settings_arm_payment_log_dedupe_keep_score( $b ) <=> arsenal_settings_arm_payment_log_dedupe_keep_score( $a );
				if ( 0 !== $score_cmp ) {
					return $score_cmp;
				}

				return (int) $a['arm_log_id'] <=> (int) $b['arm_log_id'];
			}
		);

		$keeper = array_shift( $group_rows );
		foreach ( $group_rows as $dup_row ) {
			$dup_id = isset( $dup_row['arm_log_id'] ) ? (int) $dup_row['arm_log_id'] : 0;
			if ( $dup_id < 1 ) {
				continue;
			}

			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name from ARMember helper.
			$result = $wpdb->delete( $table, array( 'arm_log_id' => $dup_id ), array( '%d' ) );
			if ( false !== $result && (int) $result > 0 ) {
				$deleted += (int) $result;
				arsenal_settings_wc_stripe_arm_cron_log(
					'payment_log_duplicate_removed',
					array(
						'dedupe_key'       => $key,
						'kept_arm_log_id'  => isset( $keeper['arm_log_id'] ) ? (int) $keeper['arm_log_id'] : 0,
						'removed_arm_log_id' => $dup_id,
						'user_id'          => isset( $keeper['arm_user_id'] ) ? (int) $keeper['arm_user_id'] : 0,
						'plan_id'          => isset( $keeper['arm_plan_id'] ) ? (int) $keeper['arm_plan_id'] : 0,
					)
				);
			}
		}
 ...
 * @return bool
 */
function arsenal_settings_arm_user_has_recent_paid_wc_stripe_success_log( $user_id, $days = 7 ) {
	global $wpdb;

	$user_id = (int) $user_id;
	$days    = max( 1, (int) $days );
	if ( $user_id < 1 || ! function_exists( 'wc_get_order' ) ) {
		return false;
	}
	...
	$rows = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT `arm_transaction_id`, `arm_extra_vars` FROM `{$table}` WHERE `arm_user_id` = %d AND `arm_payment_gateway` = %s AND `arm_transaction_status` = %s AND `arm_created_date` >= %s ORDER BY `arm_log_id` DESC LIMIT 15",
			$user_id,
			'woocommerce',
			'success',
			$since
	...
	foreach ( $rows as $row ) {
		$extra = arsenal_settings_arm_payment_log_extra_vars_array( isset( $row['arm_extra_vars'] ) ? $row['arm_extra_vars'] : '' );
		$oid   = isset( $extra['woocommerce_order_id'] ) ? (int) $extra['woocommerce_order_id'] : 0;
		if ( $oid > 0 ) {
			$order = wc_get_order( $oid );
			if ( $order && arsenal_settings_wc_stripe_order_is_paid_or_verified( $order ) ) {
				return true;
			}
		}

		$tid = isset( $row['arm_transaction_id'] ) ? (string) $row['arm_transaction_id'] : '';
		if ( preg_match( '/^\d+$/', $tid ) ) {
			$order = wc_get_order( (int) $tid );
			if ( $order && 'stripe' === (string) $order->get_payment_method() && arsenal_settings_wc_stripe_order_is_paid_or_verified( $order ) ) {
				return true;
			}
		}
	}
...
function arsenal_settings_repair_failed_arm_woocommerce_log_from_stripe( array $failed_row ) {
	...
	$table        = arsenal_settings_get_armember_payment_log_table();
	$failed_extra = arsenal_settings_arm_payment_log_extra_vars_array( isset( $failed_row['arm_extra_vars'] ) ? $failed_row['arm_extra_vars'] : '' );
	$wc_order_id  = isset( $failed_extra['woocommerce_order_id'] ) ? (int) $failed_extra['woocommerce_order_id'] : 0;

	$duplicate_success_id = arsenal_settings_arm_find_existing_wc_stripe_success_payment_log_id(
		$table,
		$user_id,
		$plan_id,
		$wc_order_id,
		isset( $stripe_match['latest_charge_id'] ) ? (string) $stripe_match['latest_charge_id'] : '',
		isset( $stripe_match['payment_intent_id'] ) ? (string) $stripe_match['payment_intent_id'] : '',
		isset( $stripe_match['subscription_id'] ) ? (string) $stripe_match['subscription_id'] : '',
	...
	if ( 0 === $duplicate_success_id ) {
		$payment_datetime_probe = arsenal_settings_resolve_arm_payment_datetime_from_stripe_match( $stripe_match, $wc_order_id );
		if ( '' !== $payment_datetime_probe ) {
			$amount = isset( $failed_row['arm_amount'] ) ? (float) $failed_row['arm_amount'] : 0;
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name from ARMember helper.
			$duplicate_success_id = (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT `arm_log_id` FROM `{$table}` WHERE `arm_user_id` = %d AND `arm_plan_id` = %d AND `arm_payment_gateway` = %s AND `arm_transaction_status` = %s AND `arm_created_date` = %s AND `arm_amount` = %f ORDER BY `arm_log_id` ASC LIMIT 1",
					$user_id,
					$plan_id,
					'woocommerce',
					'success',
					$payment_datetime_probe,
					$amount
				)
			);
		}
	...
	if ( $duplicate_success_id > 0 && $duplicate_success_id !== $log_id ) {
		...
		arsenal_settings_wc_stripe_arm_cron_log(
			$ok ? 'failed_log_repair_duplicate_removed' : 'failed_log_repair_duplicate_remove_error',
			array(
				'arm_log_id'           => $log_id,
				'user_id'              => $user_id,
				'plan_id'              => $plan_id,
				'kept_arm_log_id'      => $duplicate_success_id,
...
function arsenal_settings_sync_wc_stripe_order_to_arm_payment_log( $order_id, $cron_log = false ) {
	...
	$stripe_subscription = (string) arsenal_settings_wc_order_meta( $order, '_stripe_subscription_id' );
	if ( '' === $stripe_subscription ) {
		$stripe_subscription = (string) arsenal_settings_wc_order_meta( $order, '_subscription_renewal' );
	}
	$inserted       = 0;
	...
	foreach ( $plan_ids as $plan_id ) {
		...
		$existing_log_id = arsenal_settings_arm_find_existing_wc_stripe_success_payment_log_id(
			$table,
			$user_id,
			(int) $plan_id,
			$order_id,
...
/**
 * Whether an ARMember plan row is past due (ARMember uses next due + 24 hours).
 *
 * @param array $plan_data arm_user_plan_{id} meta.
 * @param int   $grace_seconds Grace period after due timestamp (default 24h).
 * @return bool
 */
function arsenal_settings_arm_plan_next_due_is_overdue( array $plan_data, $grace_seconds = DAY_IN_SECONDS ) {
	...
	return ( $next_due + $grace_seconds ) < (int) current_time( 'timestamp' );
}

/**
 * Whether a member plan is paid via Stripe bank direct debit (ARMember auto-debit or WooCommerce + DD PM).
 *
 * @param int   $user_id   WordPress user id.
 * @param int   $plan_id   ARMember plan id.
 * @param array $dd_cache  Optional per-user cache (user id => bool).
 * @return bool
 */
function arsenal_settings_user_plan_is_stripe_direct_debit( $user_id, $plan_id, array &$dd_cache = array() ) {
	$user_id = (int) $user_id;
	$plan_id = (int) $plan_id;
	if ( $user_id < 1 || $plan_id < 1 ) {
		return false;
	...
	if ( function_exists( 'arsenal_settings_user_armember_plan_is_stripe_auto_debit' )
		&& arsenal_settings_user_armember_plan_is_stripe_auto_debit( $user_id, $plan_id ) ) {
		return true;
	}
	...
	return in_array( $gateway, array( 'woocommerce', 'stripe' ), true )
		|| ( 'stripe' === $gateway && 'manual_subscription' === $mode );
}

/**
 * Build a minimal ARMember payment-log probe row for Stripe subscription matching.
 *
 * @param int   $user_id   WordPress user id.
 * @param int   $plan_id   ARMember plan id.
 * @param array $plan_data arm_user_plan_{id} meta (optional).
 * @return array
 */
function arsenal_settings_arm_probe_row_from_plan( $user_id, $plan_id, array $plan_data = array() ) {
	$user_id = (int) $user_id;
	$plan_id = (int) $plan_id;
	if ( empty( $plan_data ) ) {
		$plan_data = get_user_meta( $user_id, 'arm_user_plan_' . $plan_id, true );
	...
	$currency = ! empty( $plan_data['arm_currency'] ) ? (string) $plan_data['arm_currency'] : '';
	...
	$hint   = $due_ts > 0 ? arsenal_settings_unix_to_arm_payment_datetime( $due_ts ) : current_time( 'mysql' );
	...
	return array(
		...
		'arm_amount'       => $amount,
		'arm_currency'     => $currency,
		'arm_created_date' => $hint,
	);
}

/**
 * Paid WooCommerce Stripe direct-debit orders for a customer since a timestamp.
 *
 * @param int $user_id   WordPress user id.
 * @param int $since_ts  Unix timestamp (UTC).
 * @return int[] Order ids newest first.
 */
function arsenal_settings_find_paid_wc_stripe_dd_orders_for_user_since( $user_id, $since_ts ) {
	$user_id  = (int) $user_id;
	$since_ts = (int) $since_ts;
	if ( $user_id < 1 || $since_ts < 1 || ! function_exists( 'wc_get_orders' ) || ! function_exists( 'arsenal_settings_dd_failure_order_is_direct_debit' ) ) {
		return array();
	}
	...
	$order_ids = wc_get_orders(
		array(
			...
			'orderby'        => 'date',
			'order'          => 'DESC',
			'status'         => array( 'completed', 'processing', 'wps_renewal' ),
			'payment_method' => 'stripe',
			'date_created'   => '>' . gmdate( 'Y-m-d\TH:i:s', $since_ts ),
			'return'         => 'ids',
		)
	);
...
/**
 * Insert a missing ARMember success payment log from a verified Stripe subscription payment.
 *
 * @param int     $user_id      WordPress user id.
 * @param int     $plan_id      ARMember plan id.
 * @param array   $stripe_match Match from Stripe.
 * @param WP_User $user         WordPress user.
 * @return int New or existing arm_log_id, or 0 on failure.
 */
function arsenal_settings_ensure_arm_success_log_from_stripe_match( $user_id, $plan_id, array $stripe_match, $user ) {
	global $wpdb, $arm_payment_gateways;
	...
	$transaction_id = isset( $stripe_match['latest_charge_id'] ) && '' !== (string) $stripe_match['latest_charge_id']
		? (string) $stripe_match['latest_charge_id']
		: ( isset( $stripe_match['payment_intent_id'] ) && '' !== (string) $stripe_match['payment_intent_id']
			? (string) $stripe_match['payment_intent_id']
			: ( isset( $stripe_match['invoice_id'] ) ? (string) $stripe_match['invoice_id'] : '' ) );
	...
	if ( $payment_log_id < 1 ) {
		arsenal_settings_api_process_log(
			'overdue_dd_stripe_log_insert_error',
			array(
				'user_id'  => $user_id,
				'plan_id'  => $plan_id,
				'db_error' => $wpdb->last_error,
...
/**
 * Reconcile one overdue direct-debit plan: sync paid WC renewals, repair failed logs, or match Stripe.
 *
 * @param int  $user_id   WordPress user id.
 * @param int  $plan_id   ARMember plan id.
 * @param bool $cron_log  Write to wc-stripe-arm-cron log.
 * @return array{status:string,order_id?:int,arm_log_id?:int}
 */
function arsenal_settings_reconcile_overdue_direct_debit_plan( $user_id, $plan_id, $cron_log = false ) {
	global $wpdb;

	$user_id = (int) $user_id;
	$plan_id = (int) $plan_id;
	if ( $user_id < 1 || $plan_id < 1 ) {
		return array( 'status' => 'skipped_invalid' );
	}
	...
	if ( ! function_exists( 'arsenal_settings_user_has_active_armember_plan' )
		...
		return array( 'status' => 'skipped_inactive' );
	}

	$dd_cache = array();
	if ( ! arsenal_settings_user_plan_is_stripe_direct_debit( $user_id, $plan_id, $dd_cache ) ) {
		return array( 'status' => 'skipped_not_direct_debit' );
	}

	$plan_data = get_user_meta( $user_id, 'arm_user_plan_' . $plan_id, true );
	$plan_data = is_array( $plan_data ) ? $plan_data : array();
	if ( ! arsenal_settings_arm_plan_next_due_is_overdue( $plan_data ) ) {
		return array( 'status' => 'skipped_not_overdue' );
	}

	$next_due = isset( $plan_data['arm_next_due_payment'] ) ? (int) $plan_data['arm_next_due_payment'] : 0;
	$since_ts = max( 0, $next_due - DAY_IN_SECONDS );
	...
	foreach ( arsenal_settings_find_paid_wc_stripe_dd_orders_for_user_since( $user_id, $since_ts ) as $order_id ) {
		...
		$order_plan_ids = arsenal_settings_get_arm_plan_ids_for_wc_order( $order );
		if ( ! in_array( $plan_id, $order_plan_ids, true ) ) {
			continue;
		}
		arsenal_settings_sync_wc_stripe_order_to_arm_payment_log( $order_id, $cron_log );
		if ( $cron_log ) {
			arsenal_settings_wc_stripe_arm_cron_log(
				'overdue_dd_wc_order_synced',
				array(
					'user_id'  => $user_id,
					'plan_id'  => $plan_id,
					'order_id' => $order_id,
	...
	$plan_data = is_array( $plan_data ) ? $plan_data : array();
	if ( ! arsenal_settings_arm_plan_next_due_is_overdue( $plan_data ) ) {
		return array( 'status' => 'reconciled_wc' );
	}

	$table = arsenal_settings_get_armember_payment_log_table();
	// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name from ARMember helper.
	$failed_row = $wpdb->get_row(
		$wpdb->prepare(
			"SELECT * FROM `{$table}` WHERE `arm_user_id` = %d AND `arm_plan_id` = %d AND `arm_payment_gateway` = %s AND `arm_transaction_status` = %s AND `arm_created_date` >= %s ORDER BY `arm_log_id` DESC LIMIT 1",
			$user_id,
			$plan_id,
			'woocommerce',
			'failed',
	...
	if ( is_array( $failed_row ) && arsenal_settings_repair_failed_arm_woocommerce_log_from_stripe( $failed_row ) ) {
		$plan_data = get_user_meta( $user_id, 'arm_user_plan_' . $plan_id, true );
		$plan_data = is_array( $plan_data ) ? $plan_data : array();
		if ( ! arsenal_settings_arm_plan_next_due_is_overdue( $plan_data ) ) {
			return array(
				'status'     => 'reconciled_failed_log',
				'arm_log_id' => isset( $failed_row['arm_log_id'] ) ? (int) $failed_row['arm_log_id'] : 0,
			);
		}
	}
	...
	$probe        = arsenal_settings_arm_probe_row_from_plan( $user_id, $plan_id, $plan_data );
	$stripe_match = arsenal_settings_find_stripe_subscription_for_failed_arm_log( $user, $probe );
	if ( is_wp_error( $stripe_match ) ) {
		if ( $cron_log ) {
			arsenal_settings_wc_stripe_arm_cron_log(
				'overdue_dd_stripe_error',
				array(
					'user_id' => $user_id,
					'plan_id' => $plan_id,
	...
	if ( empty( $stripe_match ) ) {
		...
		return array( 'status' => 'still_overdue_unpaid' );
	}
	...
	if ( $cron_log ) {
		arsenal_settings_wc_stripe_arm_cron_log(
			'overdue_dd_reconciled_stripe',
			array(
				'user_id'      => $user_id,
				'plan_id'      => $plan_id,
				'arm_log_id'   => $log_id,
				'stripe_sub'   => isset( $stripe_match['subscription_id'] ) ? (string) $stripe_match['subscription_id'] : '',
			)
		);
	}
	...
	if ( ! arsenal_settings_arm_plan_next_due_is_overdue( $plan_data ) ) {
		return array(
			'status'     => 'reconciled_stripe',
			'arm_log_id' => $log_id,
		);
	}

	return array(
		'status'     => 'reconciled_stripe_due_stale',
		'arm_log_id' => $log_id,
	);
}

/**
 * Cron batch: scan active, non-suspended members with overdue direct-debit plans and reconcile payments.
 *
 * @param int $batch_size Users to scan per run.
 * @return array Summary counters.
 */
function arsenal_settings_cron_reconcile_overdue_direct_debit_plans( $batch_size = 25 ) {
	$batch_size = max( 5, min( 100, (int) $batch_size ) );
	$offset     = (int) get_option( 'arsenal_settings_dd_overdue_cron_offset', 0 );
	...
	$summary = array(
		...
		'reconciled_failed'    => 0,
		'reconciled_stripe'    => 0,
		'still_overdue_unpaid' => 0,
		'skipped'              => 0,
	);

	if ( empty( $users ) ) {
		update_option( 'arsenal_settings_dd_overdue_cron_offset', 0, false );
		return $summary;
	}

	arsenal_settings_wc_stripe_arm_cron_log(
		'overdue_dd_cron_start',
		array(
			'offset'     => $offset,
	...
	foreach ( $users as $user ) {
		...
		foreach ( $plan_ids as $plan_id ) {
			...
			$result = arsenal_settings_reconcile_overdue_direct_debit_plan( $user_id, $plan_id, true );
			...
			switch ( $status ) {
				...
				case 'reconciled_stripe_due_stale':
					...
					break;
				case 'still_overdue_unpaid':
					$summary['still_overdue_unpaid']++;
					break;
				default:
					$summary['skipped']++;
			}
	...
	update_option( 'arsenal_settings_dd_overdue_cron_offset', $offset + count( $users ), false );

	arsenal_settings_wc_stripe_arm_cron_log(
		'overdue_dd_cron_complete',
		$summary
	);
