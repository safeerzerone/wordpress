...
/**
 * WC_Gateway_Palmodule_PayPal_Rest class.
 *
 * @extends WC_Payment_Gateway
 */
class WC_Gateway_Palmodule_PayPal_Rest extends WC_Payment_Gateway_CC {
    ...
    public function __construct() {
        ...
        $this->method_description = __('PayPal direct credit card payments using the REST API.  This allows you to accept credit cards directly on the website.', 'palmodule-paypal-payment-for-woocoomerce');
        $this->supports = array(
            'products',
            'refunds'
        );
        ...
        $this->icon = apply_filters('woocommerce_palmodule_paypal_rest_icon', $this->icon);
    ...
    public function init_form_fields() {
        $this->form_fields = include( 'settings-palmodule-paypal-rest.php' );
    }
    ...
    public function validate_fields() {
        try {
            if (isset($_POST['wc-palmodule_paypal_rest-payment-token']) && 'new' !== $_POST['wc-palmodule_paypal_rest-payment-token']) {
                $token_id = wc_clean($_POST['wc-palmodule_paypal_rest-payment-token']);
                $token = WC_Payment_Tokens::get($token_id);
                if ($token->get_user_id() !== get_current_user_id()) {
                    throw new Exception(__('Error processing checkout. Please try again.', 'palmodule-paypal-payment-for-woocoomerce'));
                } else {
                    return true;
                }
            }
    ...
    public function init_api() {
        try {
            ...
            include_once( PALMODULE_PAYPAL_PAYMENT_FOR_WOOCOOMERCE_PLUGIN_DIR . '/includes/gateways/paypal-rest/class-wc-gateway-palmodule-paypal-rest-api-handler.php' );
            $this->rest_api_handler = new WC_Gateway_Palmodule_PayPal_Rest_API_Handler();
