...
class WC_Gateway_Palmodule_PayPal_Rest_API_Handler {
    ...
    public function getAuth() {
        try {
            ...
            $auth->setConfig(array('mode' => $this->mode, 'http.headers.PayPal-Partner-Attribution-Id' => 'mbjtechnolabs_SP', 'log.LogEnabled' => true, 'log.LogLevel' => 'DEBUG', 'log.FileName' => wc_get_log_file_path('palmodule_paypal_rest'), 'cache.enabled' => true, 'cache.FileName' => wc_get_log_file_path('palmodule_paypal_rest_cache')));
            return $auth;
        } catch (Exception $ex) {
    ...
    public function palmodule_set_card_data($card_data, $is_save = false) {
        try {
    ...
    public function palmodule_set_fundinginstrument($order_id) {
        try {
    ...
    public function palmodule_set_item_list($order_id) {
        try {
    ...
    public function palmodule_set_detail_amount($order) {
        try {
            ...
            $this->amount->setTotal(palmodule_number_format($order->get_total(), $order));
    ...
    public function palmodule_set_shipping_address($order) {
    ...
    public function palmodule_set_transaction($order) {
        try {
            ...
            $this->transaction->setAmount($this->amount)
                    ...
                    ->setNotifyUrl(apply_filters('palmodule_palmodule_paypal_rest_url', add_query_arg('palmodule_ipn_action', 'ipn', WC()->api_request_url( 'Palmodule_PayPal_Payment_For_Woocoomerce_Paypal_IPN_Handler' ))))
    ...
    public function is_request_using_save_card_data($order_id) {
        try {
            $bool = false;
            try {
                if (!empty($_POST['wc-palmodule_paypal_rest-payment-token']) && $_POST['wc-palmodule_paypal_rest-payment-token'] != 'new') {
                    $bool = true;
                }
                if ($this->is_renewal($order_id)) {
                    $bool = true;
    ...
    public function is_save_card_data() {
        try {
            $bool = false;
            try {
                if (!empty($_POST['wc-palmodule_paypal_rest-new-payment-method']) && $_POST['wc-palmodule_paypal_rest-new-payment-method'] == true) {
                    $bool = true;
                }
            } catch (Exception $ex) {
    ...
    public function create_payment_request($card_data, $order_id, $used_payment_token) {
        ...
        try {
            if ($this->is_request_using_save_card_data($order_id) == true) {
                $this->palmodule_set_card_token($order_id);
            } else {
                $this->palmodule_set_card_data($card_data);
                if ($this->is_save_card_data() == true) {
                    $this->palmodule_save_card_data_in_vault();
                }
            }
            ...
            if ($order->get_total() > 0) {
                $this->palmodule_set_fundinginstrument($order_id);
                $this->palmodule_set_payer();
                $this->palmodule_set_item_list($order_id);
                $this->palmodule_set_shipping_address($order);
                $this->palmodule_set_detail_amount($order);
                $this->palmodule_set_transaction($order);
                ...
                if ($this->payment->state == "approved") {
                    ...
                    $order->add_order_note(__('PayPal Credit Card Payments (REST) payment completed', 'palmodule-paypal-payment-for-woocoomerce'));
                    if (!empty($this->token)) {
                        $order->add_payment_token($this->token);
                    }
                    wc_reduce_stock_levels($order_id);
                    $order->payment_complete($saleId);
                    return $this->palmodule_return_checkout_order_received_url($order);
                }
            } else {
                $order->add_order_note(__('PayPal Credit Card Payments (REST) payment completed', 'palmodule-paypal-payment-for-woocoomerce'));
                if (!empty($this->token)) {
    ...
    public function palmodule_set_card_token($order_id) {
        try {
            if ($this->is_renewal($order_id)) {
                $this->restcreditcardid = get_post_meta($order_id, '_payment_tokens_id', true);
            } else {
                $token_id = wc_clean($_POST['wc-palmodule_paypal_rest-payment-token']);
    ...
    public function palmodule_save_card_data_in_woo() {
        try {
    ...
    public function palmodule_add_payment_method($card_data) {
        $this->palmodule_set_card_data($card_data, $is_save = true);
        $this->palmodule_save_card_data_in_vault();
        if ($this->card->getState() == 'ok') {
            $result = 'success';
        } else {
