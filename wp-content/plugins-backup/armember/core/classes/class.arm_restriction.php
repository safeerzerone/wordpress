...
if (!class_exists('ARM_restriction')) {
    ...
    class ARM_restriction {
        ...
        function arm_custom_posts_join($join) {
            ...
            $arm_allow_content_listing = apply_filters('arm_allow_content_listing_external', $arm_allow_content_listing);
        ...
        function arm_restriction_init() {
            ...
            if (!empty($block_urls)) {
                ...
                $request_url = $ARM_HOST_REQUEST_SECURED . $SERVER_HTTP_HOST . $SERVER_REQUEST_URI;//phpcs:ignore


                if ( !empty( $SERVER_HTTP_HOST ) && $arm_global_settings->arm_find_url_match($request_url, $block_urls)) {
        ...
        function arm_restrict_all_access($wp) {
            ...
            if ($restrict_site_access == '1') {
                ...
                $allow_page_ids = apply_filters('arm_restrict_entire_website_exclude_pages_external',$allow_page_ids);
                if ((isset($wp->request) && in_array($wp->request, $allow_page_ids))) {
                    return;
                }
                else if (!empty($restrict_site_redirect_guest_pageid) && $restrict_site_redirect_guest_pageid != 0) {
                    /* Set Guest page content */
        ...
        function arm_wp_head_redirect($arm_get_post_arr=array(), $arm_return_data=0) {
            ...
            if ('posts' == $show_on_front && is_home()) {
                ...
                $arm_default_redirection_settings = get_option('arm_redirection_settings');
                $arm_default_redirection_settings = maybe_unserialize($arm_default_redirection_settings);
                $access_rules_options = !empty($arm_default_redirection_settings['default_access_rules']) ? $arm_default_redirection_settings['default_access_rules'] : array();
        ...
        function arm_check_content_hasaccess( $membership_plans = array(), $type = ""   ) {
            ...
            if( !empty( $arm_pay_per_post_feature->isPayPerPostFeature ) ){
                // unset membership plan ID which has a paid post
                foreach($arm_user_plan as $plan_id)
                {
                    ...
                    if( !empty( $paid_post_data[0]['arm_subscription_plan_post_id'] ) ){
                        unset($arm_user_plan[array_search($plan_id, $arm_user_plan)]);
                    }
                }
            }
            ...
            if (!empty($plan)) {
                ...
                if ($type == 'show') {
                    if ($isLoggedIn) {
                        ...
                        if(empty($arm_user_plan) && in_array("-2",$plans))
                        {
                            $hasaccess = TRUE;
                        }
...
if (!function_exists('arm_new_user_notification')) {
     ...
     * @return type
     */
    function arm_new_user_notification($user_id, $plaintext_pass = '', $user_notification_status = '',$is_allow_send = true) {
        global $wp, $wpdb, $current_user, $arm_errors, $ARMember, $arm_member_forms, $arm_global_settings, $arm_email_settings;
        ...
        if ($user_register_verification == 'email' && $is_allow_send) {
