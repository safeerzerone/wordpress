...
if (!class_exists('ARM_subsctriptions')) {
    ...
    class ARM_subsctriptions{
        ...
        function arm_get_upcomming_sub_details_for_grid_func(){
            global $wpdb, $ARMember, $arm_slugs, $arm_members_class, $arm_member_forms, $arm_global_settings, $arm_subscription_plans, $arm_payment_gateways, $is_multiple_membership_feature, $arm_capabilities_global,$arm_transaction;

            $arm_activity_id = intval($_POST['activity_id']);

            $ARMember->arm_check_user_cap($arm_capabilities_global['arm_manage_subscriptions'], '1'); //phpcs:ignore --Reason:Verifying nonce
            ...
            $sql = $wpdb->prepare('SELECT act.*,am.arm_user_login FROM '.$ARMember->tbl_arm_activity.' act LEFT JOIN '.$ARMember->tbl_arm_members.' am ON act.arm_user_id = am.arm_user_id WHERE act.arm_user_id !=%d AND act.arm_action != "eot" AND act.arm_activity_id = %d',0,$arm_activity_id); //phpcs:ignore --Reason $ARMember->tbl_arm_activity and $ARMember->tbl_arm_members are a table names
            ...
            if(!empty($arm_upc_rc))
            {
                ...
                $arm_payment_gateway_txt = esc_html__('Manual','ARMember');
                ...
                $return = '<div class="arm_child_row_div"><div class="arm_child_user_data_section">';
                    $return .= '<div class="arm_view_member_left_box arm_no_border arm_margin_top_0">
                        <div class="arm_view_member_sub_title arm_padding_0 arm_margin_bottom_24">'.esc_html__('Subscription details','ARMember').'</div>
                ...
                $return = '<div class="arm_child_row_div"><div class="arm_child_user_data_section">';
                    $return .= '<div class="arm_view_member_left_box arm_no_border arm_margin_top_0">
                        <div class="arm_view_member_sub_title arm_padding_0 arm_margin_bottom_24">'.esc_html__('Subscription details','ARMember').'</div>
                            <div>'.esc_html__('Subscription details not found','ARMember').'</div>
                        </div>
                </div></div>';
            }
        ...
        function arm_get_user_activity_details_for_grid_func(){
            ...
            if(!empty($arm_logs_result)){
                ...
                $txn_gateway = !empty($payment_gateway) ? $payment_gateway : esc_html__('Manual','ARMember');
                ...
                $return = '<div class="arm_child_row_div"><div class="arm_child_user_data_section">';
                    $return .= '<div class="arm_view_member_left_box arm_no_border arm_margin_top_0">
                        <div class="arm_view_member_sub_title arm_padding_0 arm_margin_bottom_24">'.esc_html__('Transaction details','ARMember').'</div>
                        <table class="form-table">';
                        foreach($grid_columns as $mkey => $mlabel)
                        {
                            $meta_val = '';
                ...
                $return = '<div class="arm_child_row_div"><div class="arm_child_user_data_section">';
                    $return .= '<div class="arm_view_member_left_box arm_no_border arm_margin_top_0">
                        <div class="arm_view_member_sub_title arm_padding_0 arm_margin_bottom_24">'.esc_html__('Transaction details','ARMember').'</div>
                            <div>'.esc_html__('Transaction details not found','ARMember').'</div>
                        </div>
                </div></div>';
            }
            echo $return; //phpcs:ignore
        ...
        function arm_get_user_subscription_details_for_grid_func(){
            ...
            $date_format = $arm_global_settings->arm_get_wp_date_format();
            $sql = $wpdb->prepare('SELECT act.*,am.arm_user_login FROM '.$ARMember->tbl_arm_activity.' act LEFT JOIN '.$ARMember->tbl_arm_members.' am ON act.arm_user_id = am.arm_user_id WHERE act.arm_activity_id=%d',$arm_activity_id); //phpcs:ignore --Reason $ARMember->tbl_arm_members is a table name
            ...
            if(!empty($response_result))
            {
                ...
                if(!empty($canceled_date))
                {
                    $get_last_transaction_sql = $wpdb->prepare("SELECT * FROM ".$ARMember->tbl_arm_payment_log." WHERE arm_user_id=%d AND arm_plan_id=%d AND arm_created_date BETWEEN %s AND %s ORDER BY arm_log_id DESC",$user_id,$arm_plan_id,$transaction_started_date,$canceled_date); //phpcs:ignore --Reason $ARMember->tbl_arm_payment_log is a table name
                }
                else
                {
                    if(!empty($user_plan_detail['arm_trial_start']))
                    ...
                    $get_last_transaction_sql = $wpdb->prepare("SELECT * FROM ".$ARMember->tbl_arm_payment_log." WHERE arm_user_id=%d AND arm_plan_id=%d AND arm_created_date >= %s ORDER BY arm_log_id DESC",$user_id,$arm_plan_id,$transaction_started_date); //phpcs:ignore --Reason $ARMember->tbl_arm_payment_log is a table name
                }
                ...
                $class = '';
                $payment_type = !empty($user_plan_detail['arm_payment_mode']) ? $user_plan_detail['arm_payment_mode'] : 'manual';
                $arm_sub_payment_type = esc_html__('Manual','ARMember');
                $transaction_count = 0;
                if($payment_gateway != 'manual')
                {
                    $payment_types = ($payment_type != 'auto_debit_subscription') ? esc_html__('Semi Automatic','ARMember') : esc_html__('Auto Debit','ARMember')  ;
                    $class = ($payment_type != 'auto_debit_subscription') ? 'arm_semi_auto' : 'arm_auto';
                    $payment_row = $payment_gateway_text." <span class='arm_payment_types arm_font_size_12 ".$class."'>".$payment_types."</span>";
                }
                if(!empty($get_transaction_sql))
                {
                    $total_trans = count($get_transaction_sql);
                    ...
                    $return .= '<div class="arm_view_member_left_box arm_no_border arm_margin_top_0">
                        ...
                        foreach($grid_columns as $mkey => $mlabel)
                        {
                            ...
                            if($mkey == 'arm_plan_start_date'){
                                ...
                                $meta_val = number_format(floatval($get_activity_data['plan_amount']),$arm_currency_decimal,'.',',') . ' '. $arm_currency;
                            }
                                ...
                                {
                                    $status = 'suspended';
                                    $meta_val = '<span class="arm_item_status_plan cancelled"><i></i>'.esc_html__('Suspended','ARMember').'</span>';
                                }
                                else if(!empty($plan_status['status']) &&  $plan_status['status'] == 'canceled')
                                {
                                    $status = 'canceled';
                                    $arm_subscription_plans_expire = '-';
                                    $meta_val = '<span class="arm_item_status_plan cancelled"><i></i>'.esc_html__('Canceled','ARMember').'</span>';
                                }
                                else if( !empty($plan_status['status']) && $plan_status['status'] == 'expired')
                                {
                                    $status = 'expired';
                                    $arm_subscription_plans_expire = '-';
                                    $meta_val = '<span class="arm_item_status_plan expired"><i></i>'.esc_html__('Expired','ARMember').'</span>';
                                }
                                else if( !empty($plan_status['status']) && $plan_status['status'] == 'active')
                                {
                                    $status = 'active';
                                    $meta_val ='<span class="arm_item_status_plan active"><i></i>'.esc_html__('Active','ARMember').'</span>';
                                }
                            }
                            $return .= '<tr class="form-field arm_detail_expand_container_child_row">
                                <th class="arm-form-table-label">'.$mlabel.'</th>
        ...
        function get_user_all_transaction_details_for_grid(){
            ...
            if(!empty($response_result))
            {
                ...
                foreach($response_transaction_result as $transactions)
                {
                    ...
                    $membersDatas['arm_invoice_id'] = $arm_invoice_id;
                    if($arm_invoice_tax_feature == 1)
                    {
                        $log_type = ($transactions->arm_payment_gateway == 'bank_transfer') ? 'bt_log' : 'other';
                        $membersDatas['arm_invoice_id'] = '<a class="armhelptip arm_invoice_detail tipso_style" href="javacript:void(0)" data-log_id='. esc_attr($transactions->arm_log_id).' data-log_type='. esc_attr($log_type).'>' . esc_html($arm_invoice_id). '</a>';
                    }
                    ...
                    $membersDatas['arm_currency'] = number_format($transactions->arm_amount,$arm_currency_decimal,'.',',') .' '. $transactions->arm_currency;
        ...
        function arm_fetch_activity_data() {
            ...
            $grid_columns = array(
                'arm_log_id' => esc_html__('invoice ID', 'ARMember'),
                'arm_plan_id' => esc_html__('Membership', 'ARMember'),
                'arm_username' => esc_html__('Username', 'ARMember'),
                'arm_display_name' => esc_html__('Name', 'ARMember'),
            ...
            $sorting_col = (isset($_REQUEST['iSortCol_0']) && $_REQUEST['iSortCol_0'] > 0) ? intval($_REQUEST['iSortCol_0']) : 1;
            ...
            $sOrder = "";
            $orderby = !empty($data_columns[(intval($sorting_col))]['data']) ? $data_columns[(intval($sorting_col))]['data'] : '';
            if(empty($orderby))
            {
                $orderby = 'arm_log_id';
            }
            ...
            if(!empty($response_result))
            {
                ...
                foreach($response_result as $rc)
                {
                    ...
                    if($arm_invoice_tax_feature == 1)
                    {
                        $response_data[$ai][1] = "<a class='armhelptip arm_invoice_detail tipso_style' href='javacript:void(0)' data-log_id='" . esc_attr($rc->arm_log_id) . "' data-log_type='" . esc_attr($log_type) . "' title='" . esc_attr__('View Invoice', 'ARMember') . "'>".$arm_invoice_id."</a>";
                    }
                    else
                    {
                        $response_data[$ai][1] = $arm_invoice_id;
                    }
                    ...
                    if($arm_is_gift_payment)
                    {
                        ...
                        $response_data[$ai][2] = (!empty($get_gift_data->arm_subscription_plan_name)) ? $get_gift_data->arm_subscription_plan_name : '-';
                        if(!empty($get_gift_data->arm_subscription_plan_name))
                        {
                            $response_data[$ai][2].='<br>'.$membership_data;
                        }
                    }
                    else if($arm_is_post_payment)
                    {
                        ...
                        $response_data[$ai][2] = (!empty($get_post_data->arm_subscription_plan_name)) ? $get_post_data->arm_subscription_plan_name : '-';
                        if(!empty($membership_data))
                        {
                            $response_data[$ai][2].='&nbsp;'.$membership_data;
                        }
                    }
                    else
                    {
                        ...
                        {
                            $response_data[$ai][2] .= "&nbsp;<span class='arm_item_status_plan membership'>".esc_html__('Plan','ARMember')."</span>";
                        }
                    }
                    /* if(!empty($plan_data))
                    {
                    ...
                    $currency_sym = (!empty($rc->arm_currency)) ? strtoupper($rc->arm_currency) : strtoupper($global_currency);
                    $response_data[$ai][6] = number_format(floatval($rc->arm_amount),$arm_currency_decimal,'.',',') .' '. $currency_sym;
                    $payment_mode = (!empty($rc->arm_payment_mode)) ? $rc->arm_payment_mode : esc_html__('Semi Automatic','ARMember');
                    if($payment_mode == 'auto_debit_subscription')
                    {
                        $payment_mode = '<span>'.esc_html__('Auto Debit','ARMember').'</span>';
                    ...
                    else
                    {
                        ...
                        $response_data[$ai][7] = !empty($payment_gateway) ? $payment_gateway : esc_html__('Manual','ARMember');    
                    }
                    $arm_transaction_status = $rc->arm_transaction_status;
                    switch ($arm_transaction_status) {
                        case '0':
                    ...
                    if ($rc->arm_payment_gateway == 'bank_transfer' && $arm_transaction_status == 'pending') {
                        ...
                        $gridAction .= "<a class='armhelptip arm_change_btlog_status' href='javascript:void(0)' onclick=\"{$changeStatusFun}armBpopup('{$armbPopupArg}');\" data-status='1' data-log_id='" . esc_attr($transactionID) . "' title='" . esc_attr__('Approve', 'ARMember') . "'><svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none'><circle cx='12' cy='12' r='10' stroke='#617191' stroke-width='1.5'/><path d='M17 8.5L10.2251 15.5L7 12.1677' stroke='#617191' stroke-width='1.5' stroke-miterlimit='10' stroke-linecap='round' stroke-linejoin='round'/></svg></a>";
                        $gridAction .= "<a class='armhelptip arm_change_btlog_status' href='javascript:void(0)' onclick=\"{$chagneStatusFun2}armBpopup('{$armbPopupArg}');\" data-status='2' data-log_id='" . esc_attr($transactionID) . "' title='" . esc_attr__('Reject', 'ARMember') . "'><svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none'><path d='M16 8L8 16' stroke='#617191' stroke-width='1.5' stroke-miterlimit='10' stroke-linecap='round' stroke-linejoin='round'/><path d='M16 16L8 8' stroke='#617191' stroke-width='1.5' stroke-miterlimit='10' stroke-linecap='round' stroke-linejoin='round'/><circle cx='12' cy='12' r='10' stroke='#617191' stroke-width='1.5'/></a>";
                    } 
                    if( $arm_transaction_status == 'success' && $arm_invoice_tax_feature == 1 ) {
                    	$gridAction .= "<a class='armhelptip arm_invoice_detail' href='javascript:void(0)' data-log_type='" . esc_attr($log_type) . "' data-log_id='" . esc_attr($transactionID) . "' title='" . esc_attr__('View Invoice', 'ARMember') . "'><svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none'><path d='M15.4286 10.8889H8.57143M13.1429 6.44444H8.57143M15.4286 15.3333H8.57143M18.8571 2H5.14286C4.83975 2 4.54906 2.11706 4.33474 2.32544C4.12041 2.53381 4 2.81643 4 3.11111V22L6.67429 20.8889L9.33714 22L12 20.8889L14.6629 22L17.3257 20.8889L20 22V3.11111C20 2.81643 19.8796 2.53381 19.6653 2.32544C19.4509 2.11706 19.1602 2 18.8571 2Z' stroke='#617191' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/></svg></a>";//phpcs:ignore
                    }
            ...
            if(!empty($response_result))
            {
                ...
                foreach($response_result as $rc)
                {
                    ...
                    if(!empty($get_activity_data))
                    {
                        ...
                        {
                            $status = 'suspended';
                            $response_data[$ai][10] = '<span class="arm_item_status_plan cancelled"><i></i>'.esc_html__('Suspended','ARMember').'</span>';
                        }
                        else if(!empty($plan_status['status']) &&  $plan_status['status'] == 'canceled')
                        {
                            $status = 'canceled';
                            $arm_subscription_plans_expire = '-';
                            $response_data[$ai][10] = '<span class="arm_item_status_plan cancelled"><i></i>'.esc_html__('Canceled','ARMember').'</span>';
                        }
                        else if( !empty($plan_status['status']) && $plan_status['status'] == 'expired')
                        {
                            $status = 'expired';
                            $arm_subscription_plans_expire = '-';
                            $response_data[$ai][10] = '<span class="arm_item_status_plan expired"><i></i>'.esc_html__('Expired','ARMember').'</span>';
                        }
                        else if( !empty($plan_status['status']) && $plan_status['status'] == 'active')
                        {
                            $status = 'active';
                            $response_data[$ai][10] ='<span class="arm_item_status_plan active"><i></i>'.esc_html__('Active','ARMember').'</span>';
                        }
                        else{
                            $arm_subscription_plans_expire = '-';
                            $status ='';
                    ...
                    if($status == 'active')
                    {
                        ...
                        {
                            $gridAction .= "<a href='javascript:void(0)' data-refund_activity_id='" . esc_attr($activityID) . "' onclick='showConfirmBoxCallback_refund(".esc_attr($activityID).");' class='armhelptip' title='" . esc_attr__('Refund', 'ARMember') . "'><svg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'><g clip-path='url(#clip0_7792_6533" . esc_attr($activityID) . ")'><path d='M20.0007 6C18.1763 3.57111 15.2716 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C16.4776 22 20.2679 19.0571 21.5422 15' stroke='#617191' stroke-width='1.5' stroke-linecap='round'/><path d='M12 17V17.5V18' stroke='#617191' stroke-width='1.5' stroke-linecap='round'/><path d='M12 6V6.5V7' stroke='#617191' stroke-width='1.5' stroke-linecap='round'/><path d='M15 9.5C15 8.11929 13.6569 7 12 7C10.3431 7 9 8.11929 9 9.5C9 10.8807 10.3431 12 12 12C13.6569 12 15 13.1193 15 14.5C15 15.8807 13.6569 17 12 17C10.3431 17 9 15.8807 9 14.5' stroke='#617191' stroke-width='1.5' stroke-linecap='round'/><path d='M22.3827 9.95962C22.3667 10.3442 21.9404 10.5673 21.6154 10.3612L17.9583 8.04257C17.6332 7.83647 17.6532 7.35576 17.9942 7.17729L21.8307 5.16949C22.1717 4.99102 22.5781 5.24864 22.5621 5.63321L22.3827 9.95962Z' fill='#617191'/></g><defs><clipPath id='clip0_7792_6533" . esc_attr($activityID) . "'><rect width='24' height='24' fill='white'/></clipPath></defs></svg></a>"; //phpcs:ignore
                        ...
                        $gridAction .= "<a href='javascript:void(0)' data-cancel_activity_type='" . esc_attr($status) . "'  data-cancel_activity_id='" . esc_attr($activityID) . "' onclick='showConfirmBoxCallback(".esc_attr($activityID).");' class='armhelptip' title='" . esc_attr__('Cancel', 'ARMember') . "'><svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none'><path d='M16 8L8 16' stroke='#617191' stroke-width='1.5' stroke-miterlimit='10' stroke-linecap='round' stroke-linejoin='round'></path><path d='M16 16L8 8' stroke='#617191' stroke-width='1.5' stroke-miterlimit='10' stroke-linecap='round' stroke-linejoin='round'></path><circle cx='12' cy='12' r='10' stroke='#617191' stroke-width='1.5'></circle></svg></a>";//phpcs:ignore
        ...
        function arm_fetch_upcoming_subscription_data() {
            ...
            $sql = $wpdb->prepare('SELECT act.*,am.arm_user_login,
             ...
             FROM '.$ARMember->tbl_arm_activity.' act LEFT JOIN '.$ARMember->tbl_arm_members.' am ON act.arm_user_id = am.arm_user_id WHERE act.arm_user_id !=%d AND act.arm_action NOT IN ("eot","cancel_subscription")',0); //phpcs:ignore --Reason $ARMember->tbl_arm_members is a table name
            ...
            if(!empty($user_ids))
            {
                ...
                if(!empty($fetch_all_activities))
                {
                    ...
                    foreach($fetch_all_activities as $rct)
                    {
                        ...
                        //check if user is already cancelled or not
                ...
                if(!empty($response_result))
                {
                    ...
                    foreach($response_result as $rc)
                    {
                        ...
                        if(!empty($get_activity_data))
                        {
                            ...
                            if($is_gift_enabled || $is_paid_post_enabled)
                            {
                                $response_data[$ai][2] = $plan_name." &nbsp; " .$membership_data. "<br/><span class='arm_plan_style' style='margin-right:10px'>".$plan_detail."</span><br/>".$grace_period_data;
                            }
                            else
                            {
                                $response_data[$ai][2] = $plan_name . "<br/><span class='arm_plan_style'>".$plan_detail."</span><br/>".$grace_period_data;
                            }
                            $response_data[$ai][6] = $arm_subscription_plans_expire;
