...
if (!class_exists('ARM_subscription_plans')) {
    ...
    class ARM_subscription_plans {
        ...
        function arm_get_membership_plan_details_func(){
            ...
            $ARMember->arm_check_user_cap($arm_capabilities_global['arm_manage_plans'], '1',1); //phpcs:ignore 

            // $form_result = $arm_subscription_plans->arm_get_all_subscription_plans();

            $arm_plan_sql = $wpdb->prepare("SELECT * FROM `" . $ARMember->tbl_arm_subscription_plans . "` WHERE `arm_subscription_plan_is_delete`=%d AND `arm_subscription_plan_post_id`=%d AND `arm_subscription_plan_gift_status`=%d",0,0,0); //phpcs:ignore --Reason $ARMember->tbl_arm_subscription_plans is a table name
            ...
            if(!empty($sSearch))
            {
                ...
                $arm_plan_sql = $wpdb->prepare("SELECT * FROM `" . $ARMember->tbl_arm_subscription_plans . "` WHERE ".$where_bgs." `arm_subscription_plan_is_delete`=%d AND `arm_subscription_plan_post_id`=%d AND `arm_subscription_plan_gift_status`=%d",0,0,0);
            }
                ...
                foreach($form_result as $planData) {
                    ...
                    if(!empty($arm_user_array)){
                        foreach($arm_user_array as $arm_user_id => $arm_user_plans){
                            if(!empty($arm_user_plans) && is_array($arm_user_plans) && in_array($planID, $arm_user_plans)){
                                $total_users++;
                            }
                        }
                    }
                    ...
                    $grid_data[$ai][1] = esc_html(stripslashes($planObj->name)); //phpcs:ignore

                    if( $planObj->is_recurring() && isset($planObj->options['payment_cycles']) && count($planObj->options['payment_cycles']) > 1 ) {
                        $grid_data[$ai][2] = '<span class="arm_item_status_text active">' . esc_html__('Paid', 'ARMember') . '</span>
                        <a href="javascript:void(0);" class="" onclick="arm_plan_cycle('. esc_attr($planID) .')">' . esc_html__('Multiple Cycle', 'ARMember') . '</a>';
                    } else {
                    ...
                    if (current_user_can('arm_manage_plans')) {
                        ...
                        if (empty($planMembers) || $planMembers == 0) {
                            $gridAction .= $arm_global_settings->arm_get_confirm_box($planID, esc_html__("Are you sure you want to delete this plan?", 'ARMember'), 'arm_plan_delete_btn','', esc_html__('Delete', 'ARMember'), esc_attr__('Cancel', 'ARMember'), esc_attr__('Delete', 'ARMember'));
                        } else {
                            $gridAction .= $arm_global_settings->arm_get_confirm_box($planID, esc_html__("This plan has one or more subscribers. So this plan can not be deleted.", 'ARMember'), 'arm_plan_delete_btn_not arm_hide','','',esc_html__("Close",'ARMember'),esc_attr__('Delete', 'ARMember'));
                        }
                    }
                    $gridAction .= "</div>";
            ...
            $columns = esc_html__('Plan ID', 'ARMember') . ',' . esc_html__('Plan Name', 'ARMember') . ',' . esc_html__('Plan Type', 'ARMember') . ',' . esc_html__('Members', 'ARMember') . ',' . esc_html__('Wp Role', 'ARMember') . ',';
            $response = array(
                'sColumns' => $columns,
                'sEcho' => $sEcho,
        ...
        function arm_warning_notices_for_payment_gateways_func($arm_payment_gateway_notice){
            ...
            $arm_payment_gateway_notice .='<a class="arm_add_new_item_box arm_page_title_link arm_ref_info_links arm_pg_important_note" id="arm_payment_gateway_notices_link" data-arm_popup_opened="1" href="#">'. esc_html__('Important note on payment integration with automatic subscription', 'ARMember').'</a>';
        ...
        function arm_plan_cycle_action_btns_func($arm_plan_cycle_action_btns,$total_inirecurring_cycle){
            ...
            $arm_plan_cycle_action_btns = '<div class="arm_plan_payment_cycle_action_buttons arm_padding_top_10 hidden_section">
        ...
        function arm_get_plan_limit_settings_content($plan_options){
            ...
            $arm_paid_post_plan_settings .= '<div class="form-field arm_plan_price_section arm_form_main_content arm_addon_section">';
            $arm_paid_post_plan_settings .= '<div class="arm_form_header_label">'. esc_html__('Membership Plan Purchase Limit Settings','ARMember') . '</div>';
                $arm_paid_post_plan_settings .= '<div id="arm_plan_price_box_content" class="arm_plan_price_box">';
                    ...
                    $arm_paid_post_plan_settings .= '<table width="100%" class="form-table">';
                        $arm_paid_post_plan_settings .= '<tr class = "form-field form-required arm_custom_addon_row">';
                        $arm_paid_post_plan_settings .= '<th class="arm_custom_addon_label arm_padding_0 arm_display_flex">';
                                $arm_paid_post_plan_settings .= '
                                <label>'. esc_html__('Purchase limit', 'ARMember') .'</label><i class="arm_helptip_icon armfa armfa-question-circle" title="'. esc_html__('The membership Limit count will be considered on "New" and "Renew" purchase actions performed either from the admin panel or from the front-end. Admin can assign unlimited time membership plans. Also, when members pay for their next "Recurring (Renewal) Cycle" payment the membership Limit count will not be considered.', 'ARMember') .'"></i>';
                            $arm_paid_post_plan_settings .= '</th>';
                            $arm_paid_post_plan_settings .= '<td class="arm_margin_bottom_24">';
                            
                                $arm_paid_post_plan_settings .= '<input type="text" name="arm_subscription_plan_options[limit]" value="'. esc_attr($plan_purchase_limit).'" size="50" class="arm_no_paste arm_min_width_232 arm_width_33_pct arm_margin_bottom_10" data-msg-required="Label should not be blank." wfd-id="id28" aria-invalid="false" onkeypress="javascript:return ArmNumberValidation(event, this)"></br>
                                <span class="arm_font_size_14 arm-gray-500">'.esc_html__('Leave blank or 0 for unlimited membership plans.','ARMember').'</span>';
        ...
        function arm_ajax_display_plan_cycle() {
            global $arm_payment_gateways, $ARMember, $arm_capabilities_global,$arm_ajax_pattern_start,$arm_ajax_pattern_end;
            ...
            if( isset($_POST['plan_id']) && !empty($_POST['plan_id']) ) {//phpcs:ignore
                ...
                if($planObj->options['payment_cycles'] > 0) {
                    ...
                    $content .= '<table class="arm_user_edit_plan_table" cellspacing="1">';
                    ...
                    foreach ($planObj->options['payment_cycles'] as $arm_cycle) {
                        ...
                        $arm_label = !empty($arm_cycle['cycle_label']) ? stripslashes($arm_cycle['cycle_label']) : '';
	...
	function arm_member_edit_membership_plan_detail_func()
        {
            ...
            $response = array("status"=>"error","response"=>esc_html__("Something Went Wrong! Please try again","ARMember"));

            if(!empty($arm_plan_id) && (!empty($_REQUEST['arm_action']) && $_REQUEST['arm_action'] == 'edit_plan'))
            {
                $arm_plan_sql = $wpdb->prepare("SELECT * FROM `" . $ARMember->tbl_arm_subscription_plans . "` WHERE `arm_subscription_plan_id`=%d AND `arm_subscription_plan_is_delete`=%d AND `arm_subscription_plan_post_id`=%d AND `arm_subscription_plan_gift_status`=%d",$arm_plan_id,0,0,0); //phpcs:ignore --Reason $ARMemberLite->tbl_arm_subscription_plans is a table name
                ...
                $response_data['arm_plan_title'] = esc_html__('Edit Membership plan',"ARMember");
        ...
        function arm_save_subscription_plans_func() {
            global $wp, $wpdb, $arm_slugs, $ARMember, $arm_global_settings, $arm_access_rules, $arm_stripe, $arm_capabilities_global, $ARMemberAllowedHTMLTagsArray,$arm_pay_per_post_feature,$arm_ajax_pattern_start,$arm_ajax_pattern_end;
            $redirect_to = admin_url('admin.php?page=' . $arm_slugs->manage_plans);

            $ARMember->arm_check_user_cap($arm_capabilities_global['arm_manage_plans'], '1',1); //phpcs:ignore --Reason:Verifying nonce

	    $response = array("status"=>"error","message"=>esc_html__("Something went wrong!Please try again",'ARMember'));
	    ...
	    $posted_data['plan_description'] = ( ! empty( $_POST['plan_description'] ) ) ?  wp_kses($_POST['plan_description'], $ARMemberAllowedHTMLTagsArray) : ''; //phpcs:ignore
		...
		if ( $plan_type != 'free' ) {
                ...
                if ($posted_data['arm_action'] == 'add') {
                    ...
                    $response = array("status"=>"success","message"=>esc_html__("Plan has been added successfully.",'ARMember'),'arm_up_dg_plans_opt' => $arm_plan_upd_drg_li);
                    ...
                    $response = array("status"=>"success","message"=>esc_html__('Plan has been updated successfully.', 'ARMember'),'arm_up_dg_plans_opt' => '');
                }
            }
            echo arm_pattern_json_encode($response);
            die();
        ...
        function arm_ajax_stop_user_subscription($user_id=0, $plan_id=0) {
            ...
            if (isset($_POST['action']) && $_POST['action'] == 'arm_cancel_membership' && isset($_POST['type']) && $_POST['type'] == 'front') {//phpcs:ignore
                $return = apply_filters('arm_cancel_membership_modify_response_external', $return, $user_id, $plan_id, $planData);
        ...
        function arm_update_user_subscription($user_id = 0, $new_plan_id = 0, $action_by = '', $allow_trial = true, $arm_last_payment_status = 'success',$start_date='',$entry_id='', $is_plan_assigned = 0) {
            ...
            global $wp, $wpdb, $ARMember, $arm_global_settings, $arm_payment_gateways, $arm_manage_communication, $arm_members_badges, $arm_members_class, $is_multiple_membership_feature, $arm_subscription_plans, $arm_pay_per_post_feature,$arm_pro_ration_feature,$arm_debug_payment_log_id;
			...
			//modify start time of membership plan
                        $start_time = apply_filters('arm_modify_subscription_plan_start_date_time_external',$start_time,$new_plan,$newPlanData,$action_by);
                        ...
                        if ($is_update_plan) {
                            ...
                            if ($new_plan->is_recurring()) {
                                if ($new_plan->has_trial_period()) {
                                    if (isset($old_plan) && !empty($old_plan) && $mail_type != 'new_subscription') {
                                        $newPlanData['arm_completed_recurring'] = 1;
                                    } else {
        ...
        function arm_get_user_membership_detail($user_id = 0, $plan_id = 0, $action = 'new_subscription', $action_by = '') {
            ...
            if (!empty($user_id) && $user_id != 0) {
                ...
                $payment_cycle = !empty($userPlanDatameta['arm_payment_cycle']) ? $userPlanDatameta['arm_payment_cycle'] : $planData['arm_payment_cycle'];
                if (!empty($planDetail)) {
                    $plan = new ARM_Plan(0);
                    $plan->init((object) $planDetail);
                } else {
                ...
                if (!empty($changePlanTo) && $changePlanTo == $plan_id) {
                    ...
                    if(isset($plan->recurring_data['trial']) && !empty($plan->recurring_data['trial'])) {
                        ...
                        if(strtotime($plan_start_date) <= current_time( 'timestamp'))
                        {
                            if ( "D" == $plan->recurring_data['trial']['period'] ) {
                                //$day += $plan->recurring_data['trial']['interval'];
                                $plan_start_date = date('Y-m-d H:i:s', strtotime($plan_start_date . " + ".$plan->recurring_data['trial']['interval']." days"));
                            } else if ( "M" == $plan->recurring_data['trial']['period'] ) {
                                //$month += $plan->recurring_data['trial']['interval'];
                                $plan_start_date = date('Y-m-d H:i:s', strtotime($plan_start_date . " + ".$plan->recurring_data['trial']['interval']." months"));
                            } else if ( "Y" == $plan->recurring_data['trial']['period'] ) {
                                //$year += $plan->recurring_data['trial']['interval'];
                                $plan_start_date = date('Y-m-d H:i:s', strtotime($plan_start_date . " + ".$plan->recurring_data['trial']['interval']." years"));
                            }
                        }

                        $membershipData['start'] = strtotime($plan_start_date);
                    ...
                    foreach ($historyRecords as $mh) {
                        ...
                        if (in_array($mh['arm_item_id'], $user_plans) && !in_array($mh['arm_item_id'], $item_id_arrray )) {
                                ...
                                if(in_array($mh['arm_item_id'], $user_suspended_plans)){
                                    $plan_name .= ' <span style="color: red" class="arm_span_msg">(' . esc_html__('Suspended', 'ARMember') . ')</span>';
                                }
                                else{
                                    if($is_paid_post!='2')
                                    {
...
if (!class_exists('ARM_Plan')) {
    ...
    class ARM_Plan {
        ...
        public function prepare_recurring_data($arm_user_selected_payment_cycle = 0,$currency='') {
            ...
            if ($this->is_recurring()) {
                ...
                if ($arm_user_selected_payment_cycle === '') {
                    ...
                    $dataArray['rec_time'] = !empty($opt_recurring['time']) ? $opt_recurring['time'] : 1;
                } else {
                    ...
                    if ($this->is_paid() && !$this->is_lifetime() && !($this->is_recurring() && $period_options['time'] == 'infinite')) {
                        ...
                        if (($this->is_recurring() && $payment_mode == 'auto_debit_subscription') || ($this->options['access_type'] == 'finite' && $payment_type == 'one_time')) {
                            ...
                            if ($arm_subscription_plan_type == 'recurring' || ($arm_subscription_plan_type == 'paid_finite' && $expiry_type == 'joined_date_expiry')) {
                                switch ($period_options['type']) {
                                    case 'D':
                                        $num = (!empty($period_options['days'])) ? ($period_options['days'] * $num_of_recurring) : $num_of_recurring;
                                        $intervalDate = "+$num day";
                                        break;
                                    case 'W':
                                        $num = (!empty($period_options['weeks'])) ? ($period_options['weeks'] * $num_of_recurring) : ($num_of_recurring);
                                        $intervalDate = "+$num week";
                                        break;
                                    case 'M':
                                        $num = (!empty($period_options['months'])) ? ($period_options['months'] * $num_of_recurring) : ($num_of_recurring);
                                        $intervalDate = "+$num month";
                                        break;
                                    case 'Y':
                                        $num = (!empty($period_options['years'])) ? ($period_options['years'] * $num_of_recurring) : ($num_of_recurring);
	    ...
	    $arm_plan_amount = str_replace(',', '', $arm_plan_amount);
            ...
            if ($this->is_paid()) {
                if ($showPlanType) {
                    $planText .= '<span class="arm_item_status_text arm_margin_right_12 active">' . esc_html__('Paid', 'ARMember') . '</span>';
                }
                if ($this->is_lifetime()) {
                ...
                $planText = '<span class="arm_item_status_text default">' . esc_html__('Free', 'ARMember') . '</span>';
            }
            return $planText;
        }
