...
$arm_member_form_id = 101;
if (isset($_GET['arm_form_id'])) { //phpcs:ignore
   $arm_member_form_id= intval($_GET['arm_form_id']); //phpcs:ignore
...
if ( isset( $_REQUEST['arm_action'] ) && $_REQUEST['arm_action'] == 'edit_member' && ! empty( $_REQUEST['id'] ) ) { //phpcs:ignore
	...
	$arm_user_roles = $user->roles;
    if((!empty($arm_user_roles) && in_array('administrator',$arm_user_roles,true)) || (is_multisite() && is_super_admin($user_id)))
    {
        wp_safe_redirect( $cancel_url );
    }
		...
		if ( ! empty( $arm_member_form_fields ) ) {
			foreach ( $arm_member_form_fields as $fields_key => $fields_value ) {
				if(in_array($fields_value['arm_form_field_option']['type'], array('file','avatar','profile_cover'))){
					...
					if(!$ARMemberLite->is_arm_pro_active)
					{
						$fname = $ARMemberLite->arm_get_basename($user->$file_meta_key);
...
$formHiddenFields = '';
?>
<div class="wrap arm_page arm_add_member_page armPageContainer popup_wrapper">
    
    <div class="content_wrapper" id="content_wrapper">
        <div class="popup_header arm_member_popup_header_wrapper">
            <div class="page_title">
                <span
                    class="arm_add_manage_member_popup_title"><?php esc_html_e('Add New Member','armember-membership');?></span>
                <span class="arm_edit_manage_member_popup_title hidden_section"
                    style="display: none;"><?php esc_html_e('Update Member','armember-membership'); ?></span>
                <span class="arm_popup_close_btn arm_members_close_btn"></span>
                <div class="arm_submit_btn_action_container">
        ...
        <div class="arm_add_edit_member_wrapper arm_member_detail_box">
            <form method="post" id="arm_add_edit_member_form" name="arm_form"
                class="arm_add_edit_member_form arm_admin_form" enctype="multipart/form-data">
                <div class="arm_member_form_section_content">

					<?php
					$arm_register_forms_fields = '';
					echo apply_filters('arm_register_form_fields_selection',$arm_register_forms_fields); //phpcs:ignore
					?>
					<div class="arm_form_fields_main_contents">
						<div class="arm_form_fields_detail_div">
							<div class="arm_form_main_content">
								<div class="arm_form_header_label">
									<?php esc_html_e('Personal Information','armember-membership');?>
								</div>
											...
											if ( empty($_REQUEST['arm_action']) && empty($_REQUEST['id'])) { //phpcs:ignore
												if ( ! empty( $dbFormFields ) ) {
													foreach ( $dbFormFields as $meta_key => $field ) {
														...
														if ( in_array( $meta_key, $arm_member_include_fields_keys ) && ! in_array( $meta_key, array( 'section', 'roles', 'html', 'hidden', 'submit', 'repeat_email', 'social_fields','user_pass','repeat_pass' ) ) ) {
															?>
												<?php
												...
												<tr class="form-field arm_user_password_field">
													<th>
														<label
															for="arm_password"><?php ( ! empty( $amr_user_pass_lbl ) ) ? esc_html_e( $amr_user_pass_lbl ) : esc_html_e( 'Password', 'armember-membership' );//phpcs:ignore ?><?php if ( $required_class != 1 ) : ?>
													...
													<td>
														<?php
																	...
																	?>
														<div class="arm_member_form_input_passwords">
												...
												<tr class="form-field form-required">
													<th>
														<label
															for="arm_username"><?php esc_html_e( 'Username', 'armember-membership' ); ?><span
							...
							<div class="arm_form_main_content">
									<div class="arm_form_header_label"><?php esc_html_e('Set Password','armember-membership');?>
									</div>
									<div class="arm_form_guts">
										<div class="arm_page_wrap">
											<div class="arm_admin_form_content">
												...
												<div class="arm_form_fields_wrapper">
													<div class="arm_setup_forms_container">
														<?php
																if(!empty($user))
																{
																	$avatarOptions['value'] = $user->profile_cover;
																}
																echo $arm_member_forms->arm_member_form_get_fields_by_type($avatarOptions, $avatar_field_id, $arm_form_id, 'active', $armform); //phpcs:ignore ?>
														<div class="armclear"></div>
														<div>
														</div>
											</td>
						...
						<div class="arm_spacing_div"></div>
						<div class="arm_form_main_content">
							<div class="arm_form_header_label"><?php esc_html_e('Settings','armember-membership');?></div>
							<div class="arm_admin_form_content">
								<table class="form-table">
									<tr class="form-field">
										<th>
											<label
												for="arm_role"><?php esc_html_e( 'Role (Optional)', 'armember-membership' ); ?></label>
										</th>
										<td class="arm-form-table-content">
											...
											<div class="arm_setup_forms_container">
												<select id="arm_role" class="arm_chosen_selectbox"
													data-msg-required="<?php esc_attr_e( 'Select Role.', 'armember-membership' ); ?>"
													name="roles[]"
													data-placeholder="<?php esc_attr_e( 'Select Role(s)..', 'armember-membership' ); ?>"
													multiple="multiple">
													<?php if ( ! empty( $user_roles ) ) { ?>
													<?php foreach ( $user_roles as $key => $val ) { ?>
													<option class="arm_message_selectbox_op"
													...
													<?php } else { ?>
													<option value="">
														<?php esc_html_e( 'No Roles Available', 'armember-membership' ); ?>
													</option>
													<?php } ?>
												</select>
											</div>
									...
									<tr class="form-field arm_members_status_col">
										...
										<td class="arm_position_relative arm_members_status_col  arm_margin_top_10 arm_padding_0">
											...
											<div class="armswitch arm_member_status_div">
												<input type="checkbox" id="arm_primary_status_check"
													<?php checked( $primary_status, '1' ); ?> value="1"
													class="armswitch_input" name="arm_primary_status" />
												<label for="arm_primary_status_check"
													class="armswitch_label arm_primary_status_check_label"></label>
											</div>
											...
											?>
											<input type="hidden" id="arm_status_switch_val"
												value="<?php echo esc_attr($primary_status); ?>" />
											...
											<tr class="form-field arm_send_email_to_user_div_tr">
												...
												<td class="arm_members_status_col  arm_margin_top_10 arm_padding_0">
													...
													<div class="armswitch arm_send_email_to_user_div">
														<input type="checkbox" id="arm_send_email_check"
															<?php checked( $email_without_payment_status, '1' ); ?> value="1"
															class="armswitch_input" name="arm_send_email" />
														<label for="arm_send_email_check"
															class="armswitch_label arm_send_email_check_label"></label>
													</div>
												</td>
											</tr>
											<?php
				...
				<div class="arm_form_main_content">
					<div class="arm_admin_form_content">
						<table class="form-table arm_subscription_plan_table">
							<tr>
								<td colspan="2">
									<div class="arm_form_header_label"><?php esc_html_e('Membership Plan','armember-membership');?></div>
								</td>
							</tr>
							

							<tr class="form-field arm_user_plan_section">
									...
									<div class="arm_member_membership_plan_confirm_box" id="arm_confirm_box_plan_change">
										<div class="arm_text_align_left arm_membership_plan_section_div">
											<div class="arm_membership_plan_section">
												...
												<span
													class="arm_add_plan_filter_label"><?php esc_html_e('Select New Plan', 'armember-membership') ?></span>
												<dl class="arm_selectbox column_level_dd">
													<dt><span></span><input type="text" style="display:none;" value=""
											...
											<div style="display: <?php echo esc_attr($display); ?>; position: relative;"
												class="arm_plan_start_date_box">
												<div class="arm_membership_plan_section arm_width_100_pct">
													<span
								...
								<?php }
									...
									echo apply_filters('arm_manage_members_plan_list_filters',$arm_membership_plan_list_filter,$planIDs,$plansLists,$planData); //phpcs:ignore
								}?>
							</tr>
								...
								<td colspan="2">
									<div class="arm_add_member_plans_div">
										...
										<table class="arm_user_plan_table">
											<tr class="odd">
												<th class="arm_user_plan_text_th arm_user_plan_no">
													<?php esc_html_e( 'No', 'armember-membership' ); ?></th>
												<th class="arm_user_plan_text_th arm_user_plan_name">
													<?php esc_html_e( 'Membership Plan', 'armember-membership' ); ?></th>
												<th class="arm_user_plan_text_th arm_user_plan_type">
													<?php esc_html_e( 'Plan Type', 'armember-membership' ); ?></th>
												<th class="arm_user_plan_text_th arm_user_plan_start">
													<?php esc_html_e( 'Starts On', 'armember-membership' ); ?></th>
												<th class="arm_user_plan_text_th arm_user_plan_end">
													<?php esc_html_e( 'Expires On', 'armember-membership' ); ?></th>
												<th class="arm_user_plan_text_th arm_user_plan_cycle_date">
													<?php esc_html_e( 'Cycle Date', 'armember-membership' ); ?></th>
												<th class="arm_user_plan_text_th arm_user_plan_action">
													<?php esc_html_e( 'Action', 'armember-membership' ); ?></th>
											</tr>
											<?php
												...
												foreach ( $planIDs as $pID ) {
													if ( ! empty( $pID ) ) {
														...
														if ( ! empty( $planData ) ) {
															...
															if($ARMemberLite->is_arm_pro_active){
																$arm_paid_condition = apply_filters('arm_pro_paid_conditional',$arm_paid_condition,$planData);
															}
															...
															$expires_on    = ! empty( $planData['arm_expire_plan'] ) ? '<span id="arm_user_expiry_date_' . esc_attr($pID) . '" style="display: flex;"> ' . date_i18n( $date_format, $planData['arm_expire_plan'] ) . ' <img src="' . esc_attr(MEMBERSHIPLITE_IMAGES_URL) . '/grid_edit_hover_trns.png" width="26" style="margin: 0px 0 0 5px; cursor: pointer;" title="' . esc_attr__( 'Change Expiry Date', 'armember-membership' ) . '" data-plan_id="' . esc_attr($pID) . '" class="arm_edit_user_expiry_date"></span><span id="arm_user_expiry_date_box_' . esc_attr($pID) . '" style="display: none; position: relative; width: 155px;"><input type="text" value="' . esc_attr( date( 'm/d/Y', $planData['arm_expire_plan'] ) ) . '"  name="arm_subscription_expiry_date_' . esc_attr($pID) . '" class="arm_member_form_input arm_user_plan_expiry_date_picker arm_width_200 arm_min_width_200" /><img src="' . esc_attr(MEMBERSHIPLITE_IMAGES_URL) . '/cancel_date_icon.png" width="11" height="11" title="' . esc_attr__( 'Cancel', 'armember-membership' ) . '" data-plan_id="' . esc_attr($pID) . '" data-plan-expire-date="' . esc_attr( date( $arm_common_date_format, $planData['arm_expire_plan'] ) ) . '" class="arm_cancel_edit_user_expiry_date"></span>' : esc_html__( 'Never Expires', 'armember-membership' ); //phpcs:ignore
															...
															?>
											<tr class="arm_user_plan_table_tr <?php echo ( $count_plans % 2 == 0 ) ? 'even' : 'odd'; ?>"
												id="arm_user_plan_div_<?php echo intval($count_plans); ?>">
												...
												<?php
																		...
																		$additional_plan_details = apply_filters('arm_content_after_membership_plan_name_external', $additional_plan_details, $user_id, $pID, $planData); //phpcs:ignore
																		
																	?>
												<td><?php echo $planName . $arm_plan_is_suspended . $additional_plan_details; //phpcs:ignore ?>
												</td>
												<td><?php echo $planObj->new_user_plan_text( false, $arm_payment_cycle ); //phpcs:ignore ?>
												</td>
												<td><?php echo $starts_on . $trial_active; //phpcs:ignore ?></td>
												<td><?php echo $expires_on; //phpcs:ignore ?></td>
												<td><?php echo $renewal_on . $arm_payment_mode; //phpcs:ignore ?></td>
													...
													<div class="arm_position_relative arm_float_left">
														<?php
																			if ( ! in_array( $pID, $suspended_plan_ids ) && $total_recurrence != $completed_rec ) {
																				?>
														<a href="javascript:void(0)" id="arm_extend_cycle_days"
															class="arm_user_extend_renewal_date_action_btn"
															onclick="showConfirmBoxCallback('extend_renewal_date_<?php echo intval($pID); ?>');"><?php esc_html_e( 'Extend Days', 'armember-membership' ); ?></a>
														<div class="arm_confirm_box arm_member_edit_confirm_box arm_confirm_box_extend_renewal_date"
															id="arm_confirm_box_extend_renewal_date_<?php echo intval($pID); ?>">
															<div class="arm_confirm_box_body">
																<div class="arm_confirm_box_arrow"></div>
																<div class="arm_confirm_box_text arm_padding_top_0">
																	<span class="arm_font_size_15 arm_margin_bottom_5">
																		<?php esc_html_e( 'Select how many days you want to extend in current cycle?', 'armember-membership' ); ?></span>
																	<div class="arm_margin_top_10">
																		<input type='hidden'
																			id="arm_user_grace_plus_<?php echo intval($pID); ?>"
																			name="arm_user_grace_plus_<?php echo intval($pID); ?>"
																			value="0"
																			class="arm-selectpicker-input-control arm_user_grace_plus" />
																		<dl
																			class="arm_selectbox column_level_dd arm_member_form_dropdown arm_width_83">
																			<dt><span>0</span><input type="text"
																			...
																			<dd>
																				<ul
																					data-id="arm_user_grace_plus_<?php echo intval($pID); ?>">
																					<?php
																											for ( $i = 0; $i <= 30; $i++ ) {
																												?>
																					<li data-label="<?php echo intval($i); ?>"
																						data-value="<?php echo intval($i); ?>">
																						<?php echo intval($i); ?></li>
																					<?php
																											}
																											?>
																				</ul>
																...
																<div class='arm_confirm_box_btn_container'>
																	<button type="button"
																		class="arm_confirm_box_btn armemailaddbtn arm_margin_right_5"
																		onclick="hideConfirmBoxCallback();"><?php esc_html_e( 'Ok', 'armember-membership' ); ?></button>
																	<button type="button"
																		class="arm_confirm_box_btn armcancel arm_user_extend_renewal_date_cancel_btn"
																		onclick="hideUserExtendRenewalDateBoxCallback(<?php echo intval($pID); ?>);"><?php esc_html_e( 'Cancel', 'armember-membership' ); ?></button>
																</div>
															</div>
														</div>
														<?php
														...
														<a href="javascript:void(0)"
															class="arm_user_renew_next_cycle_action_btn"
															id="arm_skip_next_cycle"
															onclick="showConfirmBoxCallback('renew_next_cycle_<?php echo intval($pID); ?>');"><?php esc_html_e( 'Renew Cycle', 'armember-membership' ); ?></a>
														<div class="arm_confirm_box arm_member_edit_confirm_box arm_confirm_box_renew_next_cycle"
															id="arm_confirm_box_renew_next_cycle_<?php echo intval($pID); ?>"
															style="top:25px; right:45px; ">
															<div class="arm_confirm_box_body">
																<div class="arm_confirm_box_arrow arm_float_right"></div>
																<div class="arm_confirm_box_text_title"><?php esc_html_e('Renew Plan', 'armember-membership' );?></div>
																<div class="arm_confirm_box_text arm_padding_top_15">
																	<input type='hidden'
																		id="arm_skip_next_renewal_<?php echo intval($pID); ?>"
																		name="arm_skip_next_renewal_<?php echo intval($pID); ?>"
																		value="0" class="arm_skip_next_renewal" />
																	<?php esc_html_e( 'Are you sure you want to renew next cycle?', 'armember-membership' ); ?>
																</div>
																<div class='arm_confirm_box_btn_container'>
																	<button type="button"
																		class="arm_confirm_box_btn armemailaddbtn arm_margin_right_5"
																		onclick="RenewNextCycleOkCallback(<?php echo intval($pID); ?>)"><?php esc_html_e( 'Ok', 'armember-membership' ); ?></button>
																	<button type="button"
																		class="arm_confirm_box_btn armcancel arm_user_renew_next_cycle_cancel_btn"
																		onclick="hideUserRenewNextCycleBoxCallback(<?php echo intval($pID); ?>);"><?php esc_html_e( 'Cancel', 'armember-membership' ); ?></button>
																</div>
															</div>
														</div>
														<?php
																	...
																	{
																		$arm_renewal_button_section = '';
																		echo apply_filters('arm_pro_paid_finite_plan_renewal_section',$arm_renewal_button_section,$pID); //phpcs:ignore
																	}

																	if ( in_array( $pID, $suspended_plan_ids ) ) {
																		?>
														<input type="hidden" name="arm_user_suspended_plan[]"
															value="<?php echo intval($pID); ?>"
															id="arm_user_suspended_plan_<?php echo intval($pID); ?>" />
														<?php
																	}
																		...
																		echo $additional_plan_action; //phpcs:ignore

																		$arm_multiple_membership_plan_renewal = '';
																		echo apply_filters('arm_multiple_membership_plan_renewal', $arm_multiple_membership_plan_renewal, $pID, $planData, $count_plans); //phpcs:ignore
																	?>
							...
							if ( ! empty( $futurePlanIDs ) ) {	
								foreach ( $futurePlanIDs as $pID ) {
									if ( ! empty( $pID ) ) {
										...
										$expires_on        = ! empty( $planData['arm_expire_plan'] ) ? '<span id="arm_user_expiry_date_' . esc_attr($pID) . '" style="display: flex;">' . date_i18n( $date_format, $planData['arm_expire_plan'] ) . ' <img src="' . esc_attr(MEMBERSHIPLITE_IMAGES_URL) . '/edit_icon.svg" width="20" style="margin: -4px 0 0 5px; cursor: pointer;" title="' . esc_attr__( 'Change Expiry Date', 'armember-membership' ) . '" data-plan_id="' . esc_attr($pID) . '" class="arm_edit_user_expiry_date"></span><span id="arm_user_expiry_date_box_' . esc_attr($pID) . '" style="display: none; position: relative; width: 155px;"><input type="text" value="' . esc_attr(date( 'm/d/Y', $planData['arm_expire_plan'] )) . '"  name="arm_subscription_expiry_date_' . esc_attr($pID) . '" class="arm_member_form_input arm_user_plan_expiry_date_picker" style="width: 120px; min-width: 120px;"/><img src="' . esc_attr(MEMBERSHIPLITE_IMAGES_URL) . '/cancel_date_icon.png" width="11" height="11" title="' . esc_attr__( 'Cancel', 'armember-membership' ) . '" data-plan_id="' . esc_attr($pID) . '" data-plan-expire-date="' . esc_attr(date( $arm_common_date_format, $planData['arm_expire_plan'] )) . '" class="arm_cancel_edit_user_expiry_date"></span>' : esc_html__( 'Never Expires', 'armember-membership' ); //phpcs:ignore
										$renewal_on        = ! empty( $planData['arm_next_due_payment'] ) ? date_i18n( $date_format, $planData['arm_next_due_payment'] ) : '-';
										$trial_starts      = ! empty( $planData['arm_trial_start'] ) ? $planData['arm_trial_start'] : '';
										$trial_ends        = ! empty( $planData['arm_trial_end'] ) ? $planData['arm_trial_end'] : '';
										$arm_payment_mode  = ( $planData['arm_payment_mode'] == 'auto_debit_subscription' ) ? '<br/>(' . esc_html__( 'Auto Debit', 'armember-membership' ) . ')' : '';
										...
										?>
											<tr class="arm_user_plan_table_tr <?php echo ( $count_plans % 2 == 0 ) ? 'even' : 'odd'; ?>"
												id="arm_user_future_plan_div_<?php echo intval($count_plans); ?>">
												...
												<td><?php echo $starts_on . $trial_active; //phpcs:ignore ?></td>
												<td><?php echo $expires_on; //phpcs:ignore ?></td>
												<td><?php echo $renewal_on . $grace_message . $arm_payment_mode; //phpcs:ignore ?>
												</td>

												<td>
													<input type="hidden" name="arm_user_future_plan[]"
														value="<?php echo intval($pID); ?>"
														id="arm_user_future_plan_<?php echo intval($pID); ?>" />
													<?php 
													$arm_multiple_membership_plan_renewal = '';
														echo apply_filters('arm_multiple_membership_plan_renewal', $arm_multiple_membership_plan_renewal, $pID, $planData, $count_plans); //phpcs:ignore
													?>
							...
							<tr>
								<td colspan="2">
									<div class="arm-note-message --alert">
										<p><?php esc_html_e( 'Note: All the actions like add new plan, change plan status, renew cycle, extend days, delete plan will be applied only after save button is clicked at the bottom of this page.', 'armember-membership' ); ?></p>
									</div>
								</td>
							</tr>
				...
				if($ARMemberLite->is_arm_pro_active)
				{
					echo '<div class="arm_spacing_div"></div>';
					$arm_add_edit_member_additional_data = '';
					echo apply_filters('arm_add_edit_member_additional_data',$arm_add_edit_member_additional_data,$planID,$planIDs,$user_id); //phpcs:ignore
				}?>
				...
				<div class="arm_spacing_div"></div>
				<div class="arm_form_main_content">
					<div class="arm_form_header_label"><?php esc_html_e('Social Fields','armember-membership');?></div>
					<div class="form-table arm_social_feature_tbl">
						<div class="arm_social_feature_fields_section">
							<div class="arm_social_form_fields">
								<div class="arm_social_account_field_lbl">
									<span class="arm_margin_bottom_12 arm_display_block arm-black-350 arm_font_size_16"><?php esc_html_e( 'Add Social Accounts', 'armember-membership' ); ?></span>
									<div class="arm-form-table-content arm_margin_top_12">
										<div class="arm_setup_forms_container arm_member_social_ac_selection_container">
											<select id="arm_member_social_ac_selection" class="arm_chosen_selectbox arm_member_social_ac_selection"  name="arm_member_social_ac_selection[]" data-placeholder="<?php esc_html_e('Select Social Account','armember-membership');?>" multiple="multiple">
												<?php
												foreach ( $socialProfileFields as $spfKey => $spfLabel ) {
													echo '<option class="arm_message_selectbox_op" value="' . esc_attr($spfKey) . '" data-label="'.strip_tags( stripslashes( $spfLabel ) ).'">' . strip_tags( stripslashes( $spfLabel ) ) . '</option>'; //phpcs:ignore
												}
												?>
											</select>
										</div>
                ...
                <?php 
					...
					echo $outside_field_content = apply_filters('arm_add_fields_in_admin_before_save_button', $outside_field_content, $user_id); //phpcs:ignore
				?>
        ...
        <div class="arm_submit_btn_container">
            ...
            <button class="greensavebtn arm_members_save_btn" type="submit"><?php esc_html_e('Save', 'armember-membership'); ?></button>
            <?php echo $formHiddenFields; //phpcs:ignore ?>
            <?php $wpnonce = wp_create_nonce( 'arm_wp_nonce' );?>
            <input type="hidden" name="arm_wp_nonce" value="<?php echo esc_attr($wpnonce);?>" />
        </div>
...
<div class="arm_member_plan_failed_payment_popup popup_wrapper">
    ...
    <div class="popup_header">
        ...
        <span class="add_rule_content"><?php esc_html_e( 'Total Skipped Cycles Of', 'armember-membership' ); ?> <span
                class="arm_failed_payment_plan_name"></span></span>
    </div>
    <div class="popup_content_text arm_member_plan_failed_payment_popup_text arm_text_align_center">

        <div class="arm_width_100_pct" style=" margin: 45px auto;"> <img
                src="<?php echo esc_attr(MEMBERSHIPLITE_IMAGES_URL) . '/arm_loader.gif'; //phpcs:ignore ?>"></div>
