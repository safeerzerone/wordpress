...
class arm_membership_change_password_element_shortcode extends Widget_Base
{
    ...
    {
        return 'arm-change-password-element-shortcode';
    }

    public function get_title()
    {
        return esc_html('ARMember Change Password','armember-membership').'<style>
        .arm_element_icon{
    ...
    protected function register_controls()
    {
        ...
        $this->start_controls_section(
			'section_content',
			[
				'label' => esc_html__( 'ARMember Change Password', 'armember-membership' ),
			]
		);
        $this->add_control(
			'title',
		...
		$forms = $wpdb->get_results( $wpdb->prepare("SELECT * FROM `" . $ARMemberLite->tbl_arm_forms . "` WHERE `arm_form_type`=%s ORDER BY `arm_form_id` ASC",'change_password'), ARRAY_A); //phpcs:ignore --Reason $ARMemberLite->tbl_arm_forms is a table
		$default = $cnt = 0;
		if(!empty($forms)){
		...
		{
			$arm_form_type = array('arm_form' => esc_html__( 'Internal', 'armember-membership' ),'popup' =>esc_html__( 'Modal (popup) Window', 'armember-membership' ));
		}
		else{
			$arm_form_type = array('arm_form' => esc_html__( 'Internal', 'armember-membership' ));
		}
		$this->add_control(
			'arm_frm_type',
			[
			...
			$arm_shortcode='';
				if(isset($settings['arm_frm_type']) && $settings['arm_frm_type']=="popup")
				{
					if (isset($settings['arm_model_trigger_type']) && ($settings['arm_model_trigger_type'] == 'link' || $settings['arm_model_trigger_type'] == 'button' || $settings['arm_model_trigger_type'] == 'onload')) {
						...
						echo  do_shortcode('[arm_form id="'.$settings['arm_shortcode_select'].'" assign_default_plan="'.$settings['arm_plan_select'].'" form_position="'.$settings['arm_frm_position'].' popup="true" link_type="'.$settings['arm_model_trigger_type'].'" link_title="'.$settings['arm_popup_label'].'" popup_height="auto" popup_width="700" link_css="" link_hover_css="" modal_bgcolor="'.$settings['arm_click_back_color'].'" overlay="'.$settings['arm_click_back_overlay'].'"]');
					}
				}
				else
				{
					echo  do_shortcode('[arm_form id="'.$settings['arm_shortcode_select'].'" assign_default_plan="'.$settings['arm_plan_select'].'" form_position="'.$settings['arm_frm_position'].'" ]');
				}
		echo '</div>';
	}
}
